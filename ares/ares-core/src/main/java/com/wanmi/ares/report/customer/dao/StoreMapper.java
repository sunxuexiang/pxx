...
public interface StoreMapper {
     ...
     * @return
     */
    @Select("select * from store where del_flag = #{delFlag}")
    @ResultMap("storeMap")
    List<Store> queryAllStoreByFlag(@Param("delFlag") boolean delFlag);

    @Select("select count(id) from store where del_flag = #{delFlag}")
     ...
     * @return 店铺名称
     */
    @Select("select store_name from store where company_info_id = #{companyId}")
