...
public class ReductionPlugin implements IGoodsListPlugin, IGoodsDetailPlugin, ITradeCommitPlugin {
    ...
    public void goodsListFilter(List<GoodsInfoVO> goodsInfos, MarketingPluginRequest request) {
        ...
        log.info("ReductionPlugin.goodsListFilter labelMap:{}", JSONObject.toJSONString(labelMap));
        goodsInfos.stream()
                .filter(goodsInfo -> request.getMarketingMap().containsKey(goodsInfo.getGoodsInfoId()))
                .forEach(goodsInfo -> {
                    //
                    List<Long> collect = request.getMarketingMap().get(goodsInfo.getGoodsInfoId()).stream()
                            .filter(marketing -> MarketingType.DISCOUNT.equals(marketing.getMarketingType())).collect(Collectors.toList()).stream().map(m -> m.getMarketingId()).collect(Collectors.toList());
                    /**
                     * 查询营销有效的商品数
                     */
//                    List<String> marketingByGoodsInfoIdsAndNum = marketingRepository.getMarketingByGoodsInfoIdsAndNum(marketingIds);
                    ...
                    request.getMarketingMap().get(goodsInfo.getGoodsInfoId()).stream()
                            .filter(marketing -> MarketingType.REDUCTION.name().equals(marketing.getMarketingType().name()))
                            .forEach(marketing -> {
                                log.info("=====================数据"+marketing);
                                ...
                                label.setAmount(labelMap.get(marketing.getMarketingId()).get(0).getAmount());
                                ...
                                log.info("ReductionPlugin.goodsListFilter goodsInfo:{}",JSONObject.toJSONString(goodsInfo));
                            });
                });
    }
    ...
    public void goodsDetailFilter(GoodsInfoDetailByGoodsInfoResponse detailResponse, MarketingPluginRequest request) {
        ...
        label.setMarketingDescList(labelMap.get(marketingObj.getMarketingId()).stream().map(marketingReductionLevelVO -> marketingReductionLevelVO.getMarketingDesc()).collect(Collectors.toList()));
...
//        else if(marketing.getSubType() == MarketingSubType.REDUCTION_FULL_ORDER ){
//            Long count = tradeItems.stream().map(TradeItemInfo::getNum).reduce(0L, (a, b) -> a + b);
//            //满数量减
//            if (count < level.getFullCount()) {
//                throw new SbcRuntimeException("K-050312");
//            }
//            amount = BigDecimal.ZERO; //满订单减默认为0
////          用于比较折扣与订单金额
////          BigDecimal reduction = level.getReduction();
//            // 若可叠加，计算优惠金额
//            if (BoolFlag.YES.equals(isOverlap) && isMax) {
...
//            Long count = tradeItems.stream().map(TradeItemInfo::getNum).reduce(0L, (a, b) -> a + b);
            ...
            // 若可叠加，计算优惠金额
            if (BoolFlag.YES.equals(isOverlap) && isMax) {
                multiple = count.divide(BigDecimal.valueOf(level.getFullCount()), 0, BigDecimal.ROUND_DOWN).longValue();
                amount = count.divide(BigDecimal.valueOf(level.getFullCount()), 0, BigDecimal.ROUND_DOWN).multiply(level.getReduction());
                reduction = amount;
            }
            if (reduction.compareTo(price) > 0) {
                amount = price;
...
//        }
    ...
    private Map<Long, List<MarketingReductionLevelVO>> getLabelMap(List<Long> marketingIds, Map<Long, MarketingResponse> marketingMap) {
        Map<Long, List<MarketingFullReductionLevel>> levelsMap = marketingFullReductionLevelRepository.findAll((root, cquery, cbuild) -> root.get("marketingId").in(marketingIds),
                        Sort.by(Arrays.asList(
                                new Sort.Order(Sort.Direction.ASC, "fullAmount"),
                                new Sort.Order(Sort.Direction.ASC, "fullCount")
//                                new Sort.Order(Sort.Direction.DESC, "reduction")
                        )))
        ...
        levelsMap.forEach((marketingId, levels) -> {
            List<MarketingReductionLevelVO> descs = levels.stream().map(level -> {
                ...
                MarketingResponse  marketingResponse=  marketingMap.get(marketingId);

                reductionLevelVO.setReduction(level.getReduction());
                if (MarketingSubType.REDUCTION_FULL_COUNT.equals(marketingResponse.getSubType()) &&
                        marketingResponse.getFullReductionLevelList().size() == 1 &&
                        marketingResponse.getFullReductionLevelList().stream().findFirst().orElse(null).getFullCount()==1) {
                    reductionLevelVO.setNumber(level.getFullCount().intValue());
                    reductionLevelVO.setMarketingDesc(String.format("立减%s元", fmt.format(level.getReduction())));
                ...
                if(MarketingSubType.REDUCTION_FULL_AMOUNT.equals(marketingResponse.getSubType())){
                    ...
                    reductionLevelVO.setMarketingDesc(String.format("满%s元减%s元", fmt.format(level.getFullAmount().setScale(0,BigDecimal.ROUND_HALF_UP)),fmt.format(level.getReduction())));
                    return reductionLevelVO;
                }
                if(MarketingSubType.REDUCTION_FULL_COUNT.equals(marketingResponse.getSubType())){
                    reductionLevelVO.setNumber(level.getFullCount().intValue());
                    reductionLevelVO.setMarketingDesc(String.format("满%s箱减%s元", fmt.format(level.getFullCount()),fmt.format(level.getReduction())));
                    return reductionLevelVO;
                }
                reductionLevelVO.setAmount(level.getFullAmount());
                reductionLevelVO.setMarketingDesc(String.format("满减%s元", fmt.format(level.getReduction())));
                return reductionLevelVO;
            }).collect(Collectors.toList());
