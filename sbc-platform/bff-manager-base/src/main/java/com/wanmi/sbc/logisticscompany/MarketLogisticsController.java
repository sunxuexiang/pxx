 ...
 * @author fcq
 */
@Api(description = "批发市场物流公司管理API", tags = "MarketLogisticsController")
@RestController
@RequestMapping(value = "/marketLogistics")
@Slf4j
public class MarketLogisticsController {

    private static final String MARKET_CHANGSHA = "长沙批发市场";
    @Autowired
...
//        }
    ...
    public BaseResponse<LogisticsCompanyPageResponse> getPage(@RequestBody @Valid LogisticsCompanyPageRequest pageReq) {
        ...
        if(pageReq.getMarketId()==null) {
            pageReq.setMarketId(Constants.BOSS_DEFAULT_MARKET_ID);
        }
        /*if(pageReq.getLogisticsType()==null) {
    ...
    public BaseResponse<LogisticsCompanyAddResponse> add(@RequestBody  @Valid LogisticsCompanyAddRequest addReq) {
        if(addReq.getMarketId()==null){
            throw new SbcRuntimeException(CommonErrorCode.SPECIFIED,"批发市场不能为空");
        }
        if(addReq.getLogisticsType()==null){
            throw new SbcRuntimeException(CommonErrorCode.SPECIFIED,"请选择物流公司类型");
        }
        //记录操作日志
        ...
        String marketName = getMarketByMarketId(addReq.getMarketId());
        if(MARKET_CHANGSHA.equals(marketName)){
            addReq.setStoreId(123457929L);
        }else {
    ...
    public BaseResponse<LogisticsCompanyAddResponse> syncLogisticsCompany(@RequestBody  @Valid LogisticsCompanySyncRequest syncRequest) {
        ...
        syncRequest.setLogisticsType(LogisticsType.THIRD_PARTY_LOGISTICS.toValue());
    ...
    public BaseResponse<LogisticsCompanyModifyResponse> modify(@RequestBody @Valid LogisticsCompanyModifyRequest modifyReq) {
        if(modifyReq.getMarketId()==null){
            throw new SbcRuntimeException(CommonErrorCode.SPECIFIED,"批发市场不能为空");
        }
        if(modifyReq.getLogisticsType()==null){
            throw new SbcRuntimeException(CommonErrorCode.SPECIFIED,"请选择物流公司类型");
        }
        //记录操作日志
    ...
    private static void stopExecute() {
        throw new SbcRuntimeException(CommonErrorCode.SPECIFIED,"批发市场物流公司暂不支持此项操作");
    }
    ...
    @RequestMapping(value = "/import/{ext}/{marketId}/{logisticsType}", method = RequestMethod.GET)
    public BaseResponse<LogisticsCompanyImportResponse> importCustomer(@PathVariable String ext,@PathVariable Long marketId,@PathVariable Integer logisticsType) {
        if (!("xls".equalsIgnoreCase(ext) || "xlsx".equalsIgnoreCase(ext))) {
            throw new SbcRuntimeException(CommonErrorCode.PARAMETER_ERROR);
        }
        if (null==marketId) {
            throw new SbcRuntimeException(CommonErrorCode.SPECIFIED,"批发市场ID为空");
        }
        if (null==logisticsType) {
            throw new SbcRuntimeException(CommonErrorCode.SPECIFIED,"物流类型为空");
        }
        ...
        excelRequest.setStoreId(Constants.BOSS_DEFAULT_STORE_ID);
        excelRequest.setLogisticsType(logisticsType);
        excelRequest.setMarketId(marketId);
