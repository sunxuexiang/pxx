...
/**
 * @description: 零售商品服务spu
 * @author: XinJiang
 * @time: 2022/3/8 16:21
 */
@Api(tags = "RetailGoodsController", description = "零售商品服务spu Api")
@RestController
@RequestMapping("/retail/goods")
public class RetailGoodsController {
...
//        }
    ...
    public BaseResponse modifyGoodsSeqNum(@RequestBody GoodsModifySeqNumRequest request) {
        ...
        GoodsByIdResponse oldGoods = retailGoodsQueryProvider.getRetailById(goodsByIdRequest).getContext();

        retailGoodsProvider.modifyRetailGoodsSeqNum(request);
        GoodsVO goodsVO = new GoodsVO();
        KsBeanUtil.copyPropertiesThird(request, goodsVO);
        esRetailGoodsInfoElasticService.modifyGoodsSeqNum(goodsVO);
    ...
    public BaseResponse onSale(@RequestBody GoodsModifyAddedStatusRequest request) {
        ...
        List<GoodsVO> goodsVOList = retailGoodsQueryProvider.listRetailByIds(goodsListByIdsRequest).getContext().getGoodsVOList();
...
//        }
        ...
        if(CollectionUtils.isNotEmpty(providerGoodsId)){
            ...
            List<GoodsVO> providerGoodsVOList = retailGoodsQueryProvider.listRetailByIds(goodsListByIdsRequest).getContext().getGoodsVOList();
        ...
        retailGoodsProvider.modifyAddedRetailStatus(request);
        //更新ES
        esRetailGoodsInfoElasticService.updateAddedStatus(AddedFlag.YES.toValue(), request.getGoodsIds(), null, null);
        ...
        if (1 == request.getGoodsIds().size()) {
            ...
            GoodsByIdResponse response = retailGoodsQueryProvider.getRetailById(goodsByIdRequest).getContext();
    ...
    public BaseResponse offSale(@RequestBody GoodsModifyAddedStatusRequest request) {
        ...
        GoodsListByIdsRequest goodsListByIdsRequest = new GoodsListByIdsRequest();
        goodsListByIdsRequest.setGoodsIds(request.getGoodsIds());
        List<GoodsVO> goodsVOList = retailGoodsQueryProvider.retailListByCondition(
        ...
        retailGoodsProvider.modifyAddedRetailStatus(request);
        //更新ES //零售商品没关联ES
        esRetailGoodsInfoElasticService.updateAddedStatus(AddedFlag.NO.toValue(), request.getGoodsIds(), null,null);
    ...
    public BaseResponse delete(@RequestBody GoodsDeleteByIdsRequest request) {
        ...
        goodsProvider.deleteRetailByIds(request);
        //关联供应商商品下架 因为零售商品没有ES
        GoodsListByIdsResponse goodsListByIdsResponse = retailGoodsQueryProvider.listByProviderGoodsId(GoodsListByIdsRequest.builder().goodsIds(request.getGoodsIds()).build()).getContext();
        if (goodsListByIdsResponse != null && CollectionUtils.isNotEmpty(goodsListByIdsResponse.getGoodsVOList())) {
            ...
            GoodsInfoListByConditionResponse goodsInfoListByConditionResponse = retailGoodsInfoQueryProvider.listByCondition(GoodsInfoListByConditionRequest.builder().goodsIds(providerOfGoodsIds).build()).getContext();
            if (goodsInfoListByConditionResponse != null && CollectionUtils.isNotEmpty(goodsInfoListByConditionResponse.getGoodsInfos())) {
                ...
                //更新上下架状态
                esRetailGoodsInfoElasticService.updateAddedStatus(AddedFlag.NO.toValue(), providerOfGoodsIds, providerOfGoodInfoIds, null);
            }
        }
        //更新ES 零售商品没有关联ES
         esRetailGoodsInfoElasticService.deleteByGoods(request.getGoodsIds());
        return BaseResponse.SUCCESSFUL();
    }
    ...
    public BaseResponse edit(@RequestBody GoodsModifyRequest request) {
        ...
        if (StringUtils.isNotEmpty(request.getGoods().getProviderGoodsId())) {
            ...
            GoodsByIdResponse context = retailGoodsQueryProvider.getRetailById(goodsByIdRequest).getContext();
            //如果找不到这个供应商商品了
        ...
        Map<String, Object> returnMap = goodsProvider.modifyRetailGoods(request).getContext().getReturnMap();

        //零售商品没有ES
        if (CollectionUtils.isNotEmpty((List<String>) returnMap.get("delStoreGoodsInfoIds"))) {
            esRetailGoodsInfoElasticService.delete((List<String>) returnMap.get("delStoreGoodsInfoIds"));
        }
        ...
        //零售商品没有ES
        esRetailGoodsInfoElasticService.deleteByGoods(Collections.singletonList(request.getGoods().getGoodsId()));
        //零售商品没有ES
        esGoodsModifyInventoryService.modifyRetailInventory(skuIds,request.getGoods().getGoodsId());
        //ares埋点-商品-后台修改商品sku,迁移至goods微服务下
    ...
    @ApiOperation(value = "批量修改商品类目(零售)")
    ...
    public BaseResponse batchModifyCate(@RequestBody GoodsBatchModifyCateRequest request) {
        ...
        goodsProvider.batchModifyRetailCate(request);
        // 刷ES
        esRetailGoodsInfoElasticService.deleteByGoods(request.getGoodsIds());

        request.getGoodsIds().forEach(goodsId ->{
            BaseResponse<GoodsInfoByGoodsIdresponse> response = retailGoodsInfoQueryProvider.getByGoodsId(DistributionGoodsChangeRequest.builder().goodsId(goodsId).build());
            List<String> skuIds = response.getContext().getGoodsInfoVOList()
                    .stream()
                    .map(GoodsInfoVO::getGoodsInfoId)
                    .filter(StringUtils::isNotEmpty)
                    .collect(Collectors.toList());
            esGoodsModifyInventoryService.modifyRetailInventory(skuIds, goodsId);
        });

        operateLogMQUtil.convertAndSend("商品", "批量修改零售类目", "批量修改零售类目: 商品ID:"
