...
@RestController("WrodEditMobileController")
@RequestMapping("/wordedit-h5")
@Slf4j
public class WrodEditMobileController {
    @Autowired
    ...
    public BaseResponse eidtWord(@RequestBody WordParamsRequest wordParamsRequest, HttpServletRequest request) throws Exception{
        ...
        // 刷新用户合同表保存的数据
        EmployeeContractResponese context = null;
        ...
        if (StringUtils.isNotEmpty(wordParamsRequest.getContractPhone()) && context == null) {
...
//            // 上传合同到法大大
            ...
            employeeId = StringUtils.isEmpty(employeeId)?wordParamsRequest.getContractPhone():employeeId;
            ...
            legalInfo.setLegalMobile(wordParamsRequest.getContractPhone());
...
//             删除xml,pdf,docx文件
    ...
    @Transactional
    @LcnTransaction
    public BaseResponse<WordParamsRequest> findByEmployeeId (@RequestBody WordParamsRequest wordParamsRequest) {
        log.info("查看客户签署信息=========={}",wordParamsRequest.toString());
        String userId = commonUtil.getUserInfo().getUserId();
        EmployeeContractResponese context1 = null;
        try {
            if (StringUtils.isNotEmpty(userId) ) {
                wordParamsRequest.setAppCustomerId(userId);
                context1 = employeeContractProvider.findByAppCustomerId(userId).getContext();
            } else {
                context1 = employeeContractProvider.findByEmployeeName(wordParamsRequest.getContractPhone()).getContext();
            }
            WordParamsRequest context = customerContractProviderTo.findByEmployeeId(wordParamsRequest).getContext();
            if (context1!= null && StringUtils.isNotEmpty(context.getContractPhone())) {
                context.setInvestemntManagerId(context1.getInvestemntManagerId());
                context.setInvestmentManager(context1.getInvestmentManager());
            }
            return BaseResponse.success(context);
        } catch (Exception e) {
            throw new SbcRuntimeException(CommonErrorCode.SPECIFIED, "用户签署正在签署多份合同，请联系运营人员");
        }
