...
@Slf4j
@RestController
@RequestMapping(value = "/boss/bulk/goods")
@Api(tags = "BossRetailGoodsController",description = "Boss散批商品服务spu")
public class BossBulkGoodsController {
    ...
    public BaseResponse<GoodsPageResponse> list(@RequestBody GoodsPageRequest pageRequest) {
        ...
        BaseResponse<GoodsPageResponse> pageResponse = bulkGoodsQueryProvider.bulkpage(pageRequest);
    ...
    public void template(@PathVariable String encrypted) {
        ...
        //操作日志记录
        operateLogMQUtil.convertAndSend("Boss零售商品服务", "下载导入商品排序模板", "操作成功");
    }
    ...
    public BaseResponse<String> upload(@RequestParam("uploadFile") MultipartFile uploadFile) {
        ...
        //操作日志记录
        operateLogMQUtil.convertAndSend("Boss零售商品服务", "上传排序文件", "操作成功:文件名称" + uploadFile.getOriginalFilename());
    ...
    public BaseResponse<Boolean> implGoods(@PathVariable String ext) {
        ...
        goodsSortImportExcelService.implBulkGoods(excelRequest);
        //操作日志记录
        operateLogMQUtil.convertAndSend("Boss零售商品服务", "确认商品排序导入", "操作成功");
    ...
    public void downErrExcel(@PathVariable String ext, @PathVariable String decrypted) {
        ...
        //操作日志记录
        operateLogMQUtil.convertAndSend("Boss零售商品服务", "下载错误文档", "操作成功");
    }
    ...
    public void exportByParams(@PathVariable String encrypted, HttpServletResponse response) {
        ...
        //操作日志记录
        operateLogMQUtil.convertAndSend("Boss零售商品服务", "导出商品列表", "操作成功");
    }
    ...
    public void exportGoodsbyTimeAndStaus(@PathVariable String encrypted, HttpServletResponse response) {
        ...
        //操作日志记录
        operateLogMQUtil.convertAndSend("Boss零售商品服务", "导出商品列表通过商品创建时间和上架状态", "操作成功");
