...
import com.wanmi.sbc.goods.api.request.brand.GoodsBrandBatchSortModifyRequest;
...
@Slf4j
@Service
public class GoodsBrandSortImportExcelService {
     ...
     */
    @Transactional
    public void implGoods(GoodsBrandSortImportExcelRequest goodsBrandRequest) {
        String ext = goodsBrandRequest.getExt();
        String filePath = HttpUtil.getProjectRealPath().concat("/").concat(Constants.EXCEL_DIR).concat("/").concat(goodsBrandRequest.getUserId()).concat(".").concat(ext);
        ...
        try (Workbook workbook = WorkbookFactory.create(file)) {
            ...
            List<GoodsBrandDTO> goodsBrandDTOS = new ArrayList<>();
            ...
            for (int rowNum = firstRowNum + 1; rowNum <= lastRowNum; rowNum++) {
                ...
                if (StringUtils.isBlank(brandName)) {
                    ...
                    // 验证数据是否重复
                    List<String> collect = brandNames.stream()
                            .filter(brandName::equals)
                            .collect(Collectors.toList());
                    if (collect.size() > 0) {
                        ExcelHelper.setError(workbook, cells[0], "品牌数据重复导入");
                        isError = true;
                    }
                    brandNames.add(brandName);
                    GoodsBrandVO goodsBrandVO = brandMap.get(brandName);
                    if (Objects.isNull(goodsBrandVO)) {
                        ExcelHelper.setError(workbook, cells[0], "品牌不存在");
                        isError = true;
                    } else {
                        // 品牌排序
                        ...
                        if( StringUtils.isBlank(sort)) {
                            ...
                            Integer brandSeqNum = Integer.valueOf(ExcelHelper.getValue(cells[1]).split("\\.")[0]);
                            if(brandSeqNum != 0) {
                                // 验证数据重复
                                List<Integer> sortList = sorts.stream()
                                        .filter(brandSeqNum::equals)
                                        .collect(Collectors.toList());
                                if (sortList.size() > 0) {
                                    ExcelHelper.setError(workbook, cells[1], "排序数据重复导入");
                                    isError = true;
                                }
                                sorts.add(brandSeqNum);
                                List<GoodsBrandVO> filterBrandVO = goodsBrandVOList.stream()
                                        .filter(vo -> brandSeqNum.equals(vo.getBrandSeqNum()) &&
                                                !vo.getBrandId().equals(goodsBrandVO.getBrandId()))
                                        .collect(Collectors.toList());
                                if (filterBrandVO.size() > 0) {
                                    ExcelHelper.setError(workbook, cells[1], "已有品牌是此排序");
                                    isError = true;
                                }
                                goodsBrandDTO.setBrandSeqNum(brandSeqNum);
                            } else {
                                goodsBrandDTO.setBrandSeqNum(null);
                            }
                        }
                        goodsBrandDTO.setBrandId(goodsBrandVO.getBrandId());
                    }
                }

                goodsBrandDTOS.add(goodsBrandDTO);
            }

            if (isError) {
                errorExcel(goodsBrandRequest.getUserId().concat(".").concat(ext), workbook);
                throw new SbcRuntimeException(GoodsImportErrorCode.CUSTOM_ERROR, new Object[]{ext});
            }
            List<GoodsBrandVO> goodsBrandVOS = goodsBrandProvider.batchModifySort(GoodsBrandBatchSortModifyRequest.builder()
                    .goodsBrandDTOList(goodsBrandDTOS)
                    .build()).getContext();
            // 更新es
            esCateBrandService.updateBrandsFromEs(goodsBrandVOS);
        } catch (SbcRuntimeException e) {
            log.error("品牌排序导入异常", e);
            throw e;
        } catch (Exception e) {
            log.error("品牌排序导入异常", e);
