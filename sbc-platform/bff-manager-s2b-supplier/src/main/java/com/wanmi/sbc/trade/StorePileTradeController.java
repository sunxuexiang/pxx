 ...
 * @Description: 店铺订单服务
 * @author: jiangxin
 * @create: 2021-10-03 9:19
 */
@Api(tags = "StoreTradeController", description = "店铺囤货订单服务API")
@RestController
@RequestMapping("/pile/trade")
public class StorePileTradeController {
    ...
    public BaseResponse cancel(@PathVariable(value = "tid") String tid) {
        ...
        try {
            ...
            //操作日志记录
            operateLogMQUtil.convertAndSend("店铺囤货订单服务", "取消订单", "取消订单：订单tid" + tid);
            ...
            if("K-050202".equals(e.getErrorCode())){
                ...
                //操作日志记录
                operateLogMQUtil.convertAndSend("店铺囤货订单服务", "异常处理逻辑", "取消订单：订单tid" + tid);
            }
            return BaseResponse.SUCCESSFUL();
        }
