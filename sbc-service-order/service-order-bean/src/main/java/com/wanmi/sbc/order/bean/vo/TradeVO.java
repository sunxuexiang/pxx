...
public class TradeVO implements Serializable {
     ...
     * 是否必须合并支付flag
     */
    @ApiModelProperty(value = "是否是散批和批发合并的订单")
     ...
     *  余额支付退款金额
     */
    @ApiModelProperty(value = "余额支付退款金额")
     ...
     * 电商拣货状态 0 未拣货 1 已拣货
     */
    @ApiModelProperty(value = "电商拣货状态 0 未拣货 1 已拣货")
    ...
    @ApiModelProperty(value = "建行对公支付上传凭证是否显示")
    private Boolean showUploadCcbPayImage;

    @ApiModelProperty(value = "建行对公支付凭证")
    private String ccbPayImg;

    private EvaluateStatus orderEvaluateStatus;
    private Integer supplierCanDelivery;
    public Integer getSupplierCanDelivery() {
        this.supplierCanDelivery = 1;
        if (this.tradeState != null && FlowState.AUDIT.equals(this.tradeState.getFlowState()) && PayState.isPaid(this.getTradeState().getPayState()) && !DeliverStatus.isShipped(this.getTradeState().getDeliverStatus())) {
            if (this.getSupplier() != null && this.getConsignee() != null && CompanyType.isSelf(this.supplier.getCompanyType()) && this.getConsignee().getProvinceId() == 430000L) {
                supplierCanDelivery = 0;
            }
        } else {
            supplierCanDelivery = 0;
        }
        return supplierCanDelivery;
    }

    private Integer canPushLogicInfoToThird=0;

    public Integer getCanPushLogicInfoToThird() {
        this.canPushLogicInfoToThird = 0;
        if (this.tradeState != null && FlowState.AUDIT.equals(this.tradeState.getFlowState()) && PayState.isPaid(this.getTradeState().getPayState()) && !DeliverStatus.isShipped(this.getTradeState().getDeliverStatus())) {
            if (this.supplier != null && CompanyType.isSelf(this.supplier.getCompanyType())) {
                canPushLogicInfoToThird = 1;
            }
        }

        return canPushLogicInfoToThird;
    }
}
