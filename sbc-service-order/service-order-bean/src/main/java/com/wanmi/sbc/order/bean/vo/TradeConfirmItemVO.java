...
public class TradeConfirmItemVO {
    ...
    public String getTradeconfirmRmark() {
        ...
        if(CollectionUtils.isNotEmpty(tradeItems)){
            goodsTotalNum = tradeItems.stream().map(TradeItemVO::getNum).reduce(Long::sum).orElse(0L);
            spuSize = tradeItems.size();
        }
        if(CollectionUtils.isNotEmpty(gifts)){
            goodsTotalNum += gifts.stream().map(TradeItemVO::getNum).reduce(Long::sum).orElse(0L);
            spuSize += gifts.size();
        }
        tradeconfirmRmark = "共"+spuSize+"种，共"+goodsTotalNum+"箱";
        return tradeconfirmRmark;
    }
    private Long mallMarketId;
