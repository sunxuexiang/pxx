...
import com.wanmi.sbc.order.pilepurchase.request.PilePurchaseRequest;
import com.wanmi.sbc.order.pilepurchaseaction.PilePurchaseAction;
import com.wanmi.sbc.order.pilepurchaseaction.PilePurchaseActionRepository;
...
@Log4j2
public class PilePurchaseService {
    ...
    public void save(PilePurchaseRequest request) {
        ...
        try {
            ...
            com.wanmi.sbc.order.pilepurchase.PilePurchase purchase = pilePurchaseRepository.findOne(request.getWhereCriteria()).orElse(null);

            sb.append(",pilePurchaseRepository.findOne end time=");
            sb.append(System.currentTimeMillis()-sTm);
            sTm = System.currentTimeMillis();
     ...
     */
    @Transactional
    public void batchSave(PilePurchaseRequest request) {
        ...
        List<com.wanmi.sbc.order.pilepurchase.PilePurchase> purchaseList = pilePurchaseRepository.findAll(PilePurchaseRequest.builder()
        ...
        goodsInfoList.forEach(goods -> {
            com.wanmi.sbc.order.pilepurchase.PilePurchase purchases = purchaseList.stream().map(purchase -> {
            ...
            request.getGoodsInfos().forEach(info -> {
                ...
                if (goods.getGoodsInfoId().equals(info.getGoodsInfoId())) {
                    ...
                    request.setGoodsSplitPrice(info.getGoodsSplitPrice());
                }
            });
            if (purchases != null) {
...
//                }
                ...
                int buyCount = Objects.isNull(countNum) ? 0 : countNum;
                addPurchase(goods, buyCount, request);
            }
        });
    ...
    public void mergePurchase(PurchaseMergeRequest request) {
        ...
        PilePurchaseRequest.setCustomerId(customer.getCustomerId());
        PilePurchaseRequest.setInviteeId(request.getInviteeId());
        PilePurchaseRequest.setSortColumn("createTime");
        PilePurchaseRequest.setSortType("desc");
        List<com.wanmi.sbc.order.pilepurchase.PilePurchase> purchaseList;
        Sort sort = PilePurchaseRequest.getSort();
        if (Objects.nonNull(sort)) {
            purchaseList = pilePurchaseRepository.findAll(PilePurchaseRequest.getWhereCriteria(), sort);
        } else {
            purchaseList = pilePurchaseRepository.findAll(PilePurchaseRequest.getWhereCriteria());
        }
        //待插入采购单商品集合
        List<GoodsInfoDTO> goodsInfos = new ArrayList<>();
        goodsInfoList.forEach(goods -> {
            // 存在重复商品 替换采购单购买数量
            Optional<com.wanmi.sbc.order.pilepurchase.PilePurchase> purchaseOptional =
                    purchaseList.stream().filter(p -> StringUtils.equals(goods.getGoodsInfoId(), p.getGoodsInfoId())).findFirst();
            if (purchaseOptional.isPresent()) {
                com.wanmi.sbc.order.pilepurchase.PilePurchase purchase = purchaseOptional.get();
                Long goodsNum =
                ...
                this.updatePurchase(purchase, PilePurchaseRequest.builder().goodsNum(goodsNum).isCover(true).build());
//                this.save(PilePurchaseRequest.builder().customerId(customer.getCustomerId()).goodsInfoId(purchase
//                .getGoodsInfoId())
//                        .goodsNum(goodsNum).isCover(true).build());
        ...
        if (goodsInfos.size() > 0) {
            if (goodsInfos.size() == Constants.PURCHASE_MAX_SIZE && purchaseList.size() > 0) {
                pilePurchaseRepository.deleteByGoodsInfoids(purchaseList.stream().map(com.wanmi.sbc.order.pilepurchase.PilePurchase::getGoodsInfoId).collect(Collectors.toList()), customer.getCustomerId(), request.getInviteeId());
                ...
                if (num >= purchaseList.size() && purchaseList.size() > 0) {
                    ...
                    pilePurchaseRepository.deleteByGoodsInfoids(purchaseList.subList(purchaseList.size() - num,
                            purchaseList.size()).stream()
                                    .map(com.wanmi.sbc.order.pilepurchase.PilePurchase::getGoodsInfoId).collect(Collectors.toList()),
            ...
            this.batchSave(PilePurchaseRequest.builder().inviteeId(request.getInviteeId()).customerId(customer.getCustomerId()).goodsInfos(goodsInfos).build());
        }
    }
     ...
     * @param request
     */
    private void addPurchase(GoodsInfoVO goodsInfo, Integer countNum, PilePurchaseRequest request) {
        log.info("===============================》开始执行方法：addPurchase，新增囤货商品购物车");
...
//        }
        purchase = new com.wanmi.sbc.order.pilepurchase.PilePurchase();
        ...
        purchase =  pilePurchaseRepository.save(purchase);
        log.info("新增囤货商品完成，返回囤货信息：{}",purchase);

        //新增囤货明细
        PilePurchaseAction action = new PilePurchaseAction();
        action.setPurchaseId(purchase.getPurchaseId());
        action.setGoodsId(goodsInfo.getGoodsId());
        action.setGoodsInfoId(goodsInfo.getGoodsInfoId());
        action.setCompanyInfoId(goodsInfo.getCompanyInfoId());
        action.setGoodsNum(request.getGoodsNum());
        action.setCustomerId(request.getCustomerId());
        action.setInviteeId(request.getInviteeId());
        action.setOrderCode(request.getOrderCode());
        action.setTakeGoodsStatus(0);
        action.setCreateTime(LocalDateTime.now());
        action.setOrderTotalPrice(request.getOrderTotalPrice());
        action.setGoodsSplitPrice(request.getGoodsSplitPrice());
        action.setPid(request.getPid());
        pilePurchaseActionRepository.save(action);
    }
     ...
     * @param request
     */
    private void updatePurchase(com.wanmi.sbc.order.pilepurchase.PilePurchase purchase, PilePurchaseRequest request) {
        log.info("===============================》开始执行方法：updatePurchase，更新囤货商品购物车");
        ...
        purchase = pilePurchaseRepository.save(purchase);
        log.info("更新囤货商品完成，返回囤货信息：{}",purchase);

        //新增囤货明细
        PilePurchaseAction action = new PilePurchaseAction();
        action.setPurchaseId(purchase.getPurchaseId());
        action.setGoodsId(purchase.getGoodsId());
        action.setGoodsInfoId(purchase.getGoodsInfoId());
        action.setCompanyInfoId(purchase.getCompanyInfoId());
        action.setGoodsNum(request.getGoodsNum());
        action.setCustomerId(request.getCustomerId());
        action.setInviteeId(request.getInviteeId());
        action.setOrderCode(request.getOrderCode());
...
//        response.setGoodsIntervalPrices(priceResponse.getGoodsIntervalPriceVOList());
     ...
     * @return
     */
    public MiniPurchaseResponse miniList(PilePurchaseRequest request, CustomerDTO customer) {
        ...
        Page<com.wanmi.sbc.order.pilepurchase.PilePurchase> purchases = pilePurchaseRepository.findAll(request.getWhereCriteria(), request.getPageRequest());
        MiniPurchaseResponse miniPurchaseResponse = new MiniPurchaseResponse();
        miniPurchaseResponse.setGoodsList(Collections.EMPTY_LIST);
        miniPurchaseResponse.setGoodsIntervalPrices(Collections.EMPTY_LIST);
        miniPurchaseResponse.setPurchaseCount(0);
        ...
        goodsInfoRequest.setGoodsInfoIds(purchases.getContent().stream().map(com.wanmi.sbc.order.pilepurchase.PilePurchase::getGoodsInfoId).collect(Collectors.toList()));
        //需要显示规格值
        goodsInfoRequest.setIsHavSpecText(Constants.yes);
        ...
        if (request.getSaasStatus() != null
                && request.getSaasStatus()) {
            miniPurchaseResponse.setPurchaseCount(pilePurchaseRepository.countByCustomerIdAndInviteeIdAndCompanyInfoId(customer.getCustomerId(), request.getInviteeId(), request.getCompanyInfoId()));
            miniPurchaseResponse.setNum(pilePurchaseRepository.queryGoodsNumByCompanyInfoId(customer.getCustomerId(), request.getInviteeId(), request.getCompanyInfoId()));
        } else {
            miniPurchaseResponse.setPurchaseCount(countGoods(customer.getCustomerId(), request.getInviteeId()));
            miniPurchaseResponse.setNum(pilePurchaseRepository.queryGoodsNum(customer.getCustomerId(), request.getInviteeId()));
        }
        return miniPurchaseResponse;
    }
...
//                            goodsInfo.getGoodsId(), goodsInfo.getGoodsInfoId());
     ...
     */
    @Transactional(rollbackFor = SbcRuntimeException.class)
    public PurchaseResponse pageList(PilePurchaseRequest request) throws SbcRuntimeException {
        request.setMinGoodsNum(0L);
        log.info("PilePurchaseService.pageList req customerId:{}",request.getCustomerId());
        StringBuffer sb = new StringBuffer();
        Long sTm = System.currentTimeMillis();
        ...
        try {
            ...
            List<com.wanmi.sbc.order.pilepurchase.PilePurchase> follows = new ArrayList<>();
            GoodsInfoViewByIdsResponse response = null;
            // 查询分页数据，防止前端对象太多，不是初始化的时候只查商品的分页数据
            Page<com.wanmi.sbc.order.pilepurchase.PilePurchase> purchasePage = null;
            log.info("PilePurchaseService.pageList sql:{}",JSONObject.toJSONString(request.getWhereCriteria()));
            // 如果是初始化的时候
            if (request.getIsRefresh()) {
                Sort sort = request.getSort();
                if (Objects.nonNull(sort)) {
                    follows = pilePurchaseRepository.findAll(request.getWhereCriteria(), sort);
                } else {
                    follows = pilePurchaseRepository.findAll(request.getWhereCriteria());
                }
                if (follows.size() == 0) {
                    return emptyResponse;
                }
                sb.append(",pilePurchaseRepository.findAll end time=");
                sb.append(System.currentTimeMillis() - sTm);
                sTm = System.currentTimeMillis();

                GoodsInfoViewByIdsRequest goodsInfoRequest = new GoodsInfoViewByIdsRequest();
                goodsInfoRequest.setGoodsInfoIds(follows.stream().map(com.wanmi.sbc.order.pilepurchase.PilePurchase::getGoodsInfoId).collect(Collectors.toList()));
                //需要显示规格值
                goodsInfoRequest.setIsHavSpecText(Constants.yes);
                ...
                for(int i = 0; i < follows.size(); i++) {
                    com.wanmi.sbc.order.pilepurchase.PilePurchase purchase = follows.get(i);
                ...
                updateList(follows);
                follows.sort(Comparator.comparing(com.wanmi.sbc.order.pilepurchase.PilePurchase::getValidSort));
                List<com.wanmi.sbc.order.pilepurchase.PilePurchase> purchaseList = follows.stream().limit(request.getPageSize()).collect(Collectors.toList());
                ...
                purchasePage = pilePurchaseRepository.findAll(request.getWhereCriteria(), request.getPageRequest());
                List<com.wanmi.sbc.order.pilepurchase.PilePurchase> purchasePageList = purchasePage.getContent();
                if (purchasePageList.size() == 0) {
                    return emptyResponse;
                }
                follows = purchasePageList;
                GoodsInfoViewByIdsRequest goodsInfoRequest = new GoodsInfoViewByIdsRequest();
                goodsInfoRequest.setGoodsInfoIds(purchasePage.getContent().stream().map(com.wanmi.sbc.order.pilepurchase.PilePurchase::getGoodsInfoId).collect(Collectors.toList()));
                //需要显示规格值
                goodsInfoRequest.setIsHavSpecText(Constants.yes);
                goodsInfoRequest.setWareId(request.getWareId());
                goodsInfoRequest.setMatchWareHouseFlag(request.getMatchWareHouseFlag());
...
//        });
            ...
            //采购单中skuId
            List<String> goodsIds = follows.stream().map(com.wanmi.sbc.order.pilepurchase.PilePurchase::getGoodsId).distinct().collect(Collectors.toList());
            List<GoodsVO> goodsList = response.getGoodses().stream()
            ...
            List<GoodsInfoVO> goodsInfoList = IteratorUtils.zip(response.getGoodsInfos(), follows,
                    (GoodsInfoVO sku, com.wanmi.sbc.order.pilepurchase.PilePurchase f) -> sku.getGoodsInfoId().equals(f.getGoodsInfoId()),
                    (GoodsInfoVO sku, com.wanmi.sbc.order.pilepurchase.PilePurchase f) -> {
            ...
            //统计商品金额
            List<BigDecimal> goodsTotal = new ArrayList<>();
            PilePurchaseRequest purchaseRequest = new PilePurchaseRequest();
            purchaseRequest.setCustomerId(request.getCustomerId());
            List<PilePurchase> purchaseList = pilePurchaseRepository.findAll(purchaseRequest.getWhereCriteria());
            Long goodsTotalCount = 0L;
            if (purchaseList.size() > 0){
                goodsTotalCount = purchaseList.stream().map(re->re.getGoodsNum()).collect(Collectors.toList()).stream().reduce(0L,Long::sum);
            }
            response.getGoodsInfos().stream().forEach(goodsInfoVO -> {
                goodsTotal.add(goodsInfoVO.getMarketPrice().multiply(BigDecimal.valueOf(goodsInfoVO.getBuyCount())));
            });
            BigDecimal goodsTotalPrice = goodsTotal.stream().reduce(BigDecimal.ZERO,BigDecimal::add).setScale(2,BigDecimal.ROUND_DOWN);

            if (response.getGoodsInfos().size() > 0 && response.getGoodses().size() > 0) {
                for (GoodsInfoVO goodsInfoVO : response.getGoodsInfos()) {
                    for (GoodsVO goodsVO : response.getGoodses()) {
                        if (goodsInfoVO.getGoodsId().equals(goodsVO.getGoodsId())) {
            ...
            if(request.getIsRefresh()) {
                List<String> goodsInfoPageIds = purchasePage.getContent().stream()
                        .map(com.wanmi.sbc.order.pilepurchase.PilePurchase::getGoodsInfoId)
                        .collect(Collectors.toList());
                List<String> goodsPageIds = purchasePage.getContent().stream()
                        .map(com.wanmi.sbc.order.pilepurchase.PilePurchase::getGoodsId)
                        .distinct()
                        .collect(Collectors.toList());
                ...
                // 购物车商品
                for(com.wanmi.sbc.order.pilepurchase.PilePurchase purchase : follows) {
     ...
     * @throws SbcRuntimeException
     */
    public PurchaseResponse list(PilePurchaseRequest request) throws SbcRuntimeException {
        ...
        try {
            ...
            //按创建时间倒序
            request.putSort("createTime", SortType.DESC.toValue());
            List<com.wanmi.sbc.order.pilepurchase.PilePurchase> follows;
            Sort sort = request.getSort();
            if (Objects.nonNull(sort)) {
                follows = pilePurchaseRepository.findAll(request.getWhereCriteria(), sort);
            } else {
...
//        });
    ...
    public void updateList(List<com.wanmi.sbc.order.pilepurchase.PilePurchase> purchases) {
        pilePurchaseRepository.saveAll(purchases);
    }
    /**
     * 异步更新选择状态
     *
    ...
    @Transactional(rollbackFor = SbcRuntimeException.class)
    @Async
    public void updateIsCheck(PilePurchaseRequest request, List<String> goodsInfoIds) throws SbcRuntimeException {
        List<com.wanmi.sbc.order.pilepurchase.PilePurchase> purchases = pilePurchaseRepository.findAll(request.getWhereCriteria());
        List<com.wanmi.sbc.order.pilepurchase.PilePurchase> savePurchases = new ArrayList<>();
        for (com.wanmi.sbc.order.pilepurchase.PilePurchase purchase : purchases) {
        ...
        if (savePurchases.size() > 0) {
            pilePurchaseRepository.saveAll(savePurchases);
        }
    }
         ...
         * @return
         */
    public List<com.wanmi.sbc.order.pilepurchase.PilePurchase> queryPurchase(String customerId, List<String> goodsInfoIds, String inviteeId) {
        //分页查询SKU信息列表
        PilePurchaseRequest request = PilePurchaseRequest.builder()
                .customerId(customerId)
                .goodsInfoIds(goodsInfoIds)
                .inviteeId(inviteeId)
                .build();
        Sort sort = request.getSort();
        List<com.wanmi.sbc.order.pilepurchase.PilePurchase> list;
        if (Objects.nonNull(sort)) {
            list = pilePurchaseRepository.findAll(request.getWhereCriteria(), sort);
        } else {
            list = pilePurchaseRepository.findAll(request.getWhereCriteria());
        }
...
//        return pilePurchaseRepository.countByCustomerId(userId);
//    }
    /*
    /**
     * 获取采购单商品数量
    ...
    public Integer countGoods(String userId, String inviteeId) {
        return pilePurchaseRepository.countByCustomerIdAndInviteeId(userId, inviteeId);
    }
    ...
    public Integer countGoodsByCompanyInfoId(String userId, String inviteeId, Long companyInfoId) {
        return pilePurchaseRepository.countByCustomerIdAndInviteeIdAndCompanyInfoId(userId, inviteeId, companyInfoId);
    }
     ...
     */
    @Transactional(rollbackFor = SbcRuntimeException.class)
    public void updateNum(PilePurchaseRequest request) {
        ...
        if (CollectionUtils.isNotEmpty(purchaseList)) {
            com.wanmi.sbc.order.pilepurchase.PilePurchase purchase = purchaseList.get(0);
            //查询囤货商品数量
            long num = purchase.getGoodsNum() - request.getGoodsNum();
            if(num > 0){
                purchase.setGoodsNum(num);
                pilePurchaseRepository.save(purchase);
            }else{
                purchase.setGoodsNum(request.getGoodsNum());
                pilePurchaseRepository.delete(purchase);
            }
        } else {
            if (countNum >= Constants.PURCHASE_MAX_SIZE) {
                throw new SbcRuntimeException(SiteResultCode.ERROR_050121, new Object[]{Constants.PURCHASE_MAX_SIZE});
            ...
            //如果数据不存在，自动加入采购单
            com.wanmi.sbc.order.pilepurchase.PilePurchase purchase = new com.wanmi.sbc.order.pilepurchase.PilePurchase();
     ...
     */
    @Transactional(rollbackFor = SbcRuntimeException.class)
    public void updatePileNum(PilePurchaseRequest request) {
        GoodsInfoVO goodsInfo = goodsInfoQueryProvider.getById(
                GoodsInfoByIdRequest.builder().goodsInfoId(request.getGoodsInfoId()).wareId(request.getWareId()).build()
        ).getContext();
        if (goodsInfo == null) {
     ...
     */
    @Transactional(rollbackFor = SbcRuntimeException.class)
    public void updateNumByIds(PilePurchaseRequest request) {
        List<com.wanmi.sbc.order.pilepurchase.PilePurchase> purchaseList = pilePurchaseRepository.findAll(PilePurchaseRequest.builder()
                .customerId(request.getCustomerId())
        ...
        if (CollectionUtils.isNotEmpty(purchaseList)) {
            ...
            List<com.wanmi.sbc.order.pilepurchase.PilePurchase> savePilePurchaseList = Lists.newArrayList();
            List<com.wanmi.sbc.order.pilepurchase.PilePurchase> deletePilePurchaseList = Lists.newArrayList();

            for (com.wanmi.sbc.order.pilepurchase.PilePurchase param : purchaseList) {
                ...
                if (goodsInfoDTO.isPresent()) {
                    long num = param.getGoodsNum() - goodsInfoDTO.get().getBuyCount();
                    if(num > 0){
                        param.setGoodsNum(num);
                        savePilePurchaseList.add(param);
                    }else{
                        deletePilePurchaseList.add(param);
                    }
                }
            }

            if(CollectionUtils.isNotEmpty(savePilePurchaseList)){
                pilePurchaseRepository.saveAll(savePilePurchaseList);
            }

            if(CollectionUtils.isNotEmpty(deletePilePurchaseList)){
                pilePurchaseRepository.deleteInBatch(deletePilePurchaseList);
            }
    ...
    public void delete(PilePurchaseRequest request) {
        pilePurchaseRepository.deleteByGoodsInfoids(request.getGoodsInfoIds(), request.getCustomerId(),
                request.getInviteeId());
     ...
     */
    @Transactional
    public void addFollow(PilePurchaseRequest queryRequest) {
        //获取所有收藏商品
        ...
        List<com.wanmi.sbc.order.pilepurchase.PilePurchase> purchaseList =
                pilePurchaseRepository.queryPurchaseByGoodsIdsAndCustomerId(queryRequest.getGoodsInfoIds(),
        ...
        purchaseList.stream().forEach(info -> {
            ...
            if (!queryRequest.getIsGift()) {
                pilePurchaseRepository.delete(info);
            }
        });
    }
    ...
    public void clearLoseGoods(String userId, DistributeChannel distributeChannel) {
        List<com.wanmi.sbc.order.pilepurchase.PilePurchase> purchaseList = pilePurchaseRepository.queryPurchaseByCustomerIdAndInviteeId(userId,
                distributeChannel.getInviteeId());
        //筛选商品id
        List<String> goodsIds = purchaseList.stream().map(com.wanmi.sbc.order.pilepurchase.PilePurchase::getGoodsInfoId).collect(Collectors.toList());
        ...
        purchaseList.forEach(item -> {
            ...
            if (Objects.nonNull(goodsInfo) && Objects.nonNull(store)) {
                ...
                if (goodsInfo.getAddedFlag() == 0 || goodsInfo.getDelFlag() == DeleteFlag.YES || goodsInfo.getAuditStatus() == CheckStatus.FORBADE || goodsInfo.getGoodsStatus() == GoodsStatus.INVALID
                    ...
                    pilePurchaseRepository.delete(item);
                }
            }
        });
    }
    ...
    public List<GoodsInfoVO> fillBuyCount(List<GoodsInfoVO> goodsInfoList, String customerId, String inviteeId) {
        //分页查询SKU信息列表
        List<com.wanmi.sbc.order.pilepurchase.PilePurchase> follows = pilePurchaseRepository.findAll(PilePurchaseRequest.builder()
     ...
     */
    public void fillBuyCount(List<GoodsInfoVO> goodsInfoList, List<com.wanmi.sbc.order.pilepurchase.PilePurchase> followList) {
        ...
        goodsInfoList.stream().forEach(goodsInfo -> {
            goodsInfo.setBuyCount(0L);
            com.wanmi.sbc.order.pilepurchase.PilePurchase purchase = followList.stream().filter(p -> goodsInfo.getGoodsInfoId().equals(p.getGoodsInfoId())).findFirst().orElse(null);
            if (Objects.nonNull(purchase)) {
...
//            }
     ...
     * @param followList    SKU商品
     */
    public void fillBuyCountInit(List<GoodsInfoVO> goodsInfoList, List<com.wanmi.sbc.order.pilepurchase.PilePurchase> followList, List<String> selectSkuids) {
        if (CollectionUtils.isEmpty(goodsInfoList) || CollectionUtils.isEmpty(followList)) {
            return;
        }
        //填充SKU的购买数
        goodsInfoList.stream().forEach(goodsInfo -> {
            ...
            if (CollectionUtils.isNotEmpty(selectSkuids)){
                if (selectSkuids.contains(goodsInfo.getGoodsInfoId())){
                    Optional<com.wanmi.sbc.order.pilepurchase.PilePurchase> first = followList.stream().filter(purchase -> goodsInfo.getGoodsInfoId().equals(purchase.getGoodsInfoId())).findFirst();
     ...
     * @author zhanglingke
     */
    public PurchaseVO wrapperVo(PilePurchase purchase) {
        if (purchase != null){
     ...
     * @return
     */
    public Long getGoodsNumByCustomerIdAndSkuId(PurchaseQueryRequest request){
        Long goodsNum = pilePurchaseRepository.getGoodsNumByCustomerIdAndGoodsInfoId(request.getCustomerId(),request.getSkuId());
        return Objects.isNull(goodsNum) ? 0L : goodsNum;
    }

    public Long getPileCountNumByCustomerId(PurchaseQueryRequest request){
        Long countNum = pilePurchaseRepository.sumGoodsNumByCustomerId(request.getCustomerId());
        return Objects.isNull(countNum) ? 0L : countNum;
    }

    public Long getGoodsNumBySkuId(String skuId){
        Long goodsNum = pilePurchaseRepository.getGoodsNumBySkuId(skuId);
        return Objects.isNull(goodsNum) ? 0L :goodsNum;
    }

    public Map<String,Long> getPileGoodsNumsBuySkuIds(List<String> skuId){
        Map<String,Long> goodsNumsMap = new HashMap<>();
        List<Object> pilePurchases = pilePurchaseRepository.getPileGoodsNumsBuySkuIds(skuId);
        if (CollectionUtils.isNotEmpty(pilePurchases)){
            pilePurchases.forEach(p->{
                log.info("===========p==============: {}", JSONObject.toJSONString(p));
                goodsNumsMap.put((String) ((Object[]) p)[0],
                                   Long.valueOf(((Object[]) p)[1].toString()));
            });
        }
        return goodsNumsMap;
    }
     ...
     * 查询当前用户所有可提货的有效囤货商品
     */
    public List<PilePurchase> getValidPilePurchaseByCustomerId(String customerId) {
        List<PilePurchase> pilePurchaseByCustomerId = pilePurchaseRepository.getPilePurchaseByCustomerId(customerId);

        /**查询真实库存*/
        GoodsWareStockListResponse context = goodsWareStockQueryProvider.getGoodsWareStockByGoodsInfoIds(
                GoodsWareStockByGoodsForIdsRequest
                        .builder()
                        .goodsForIdList(pilePurchaseByCustomerId.stream().map(p -> p.getGoodsInfoId()).collect(Collectors.toList()))
                        .build()).getContext();

        if(CollectionUtils.isEmpty(pilePurchaseByCustomerId) || Objects.isNull(context) || CollectionUtils.isEmpty(context.getGoodsWareStockVOList())){
            return Lists.newArrayList();
        }
        Map<String, BigDecimal> goodsInfoStockMap = context.getGoodsWareStockVOList().stream().collect(Collectors.toMap(GoodsWareStockVO::getGoodsInfoId, GoodsWareStockVO::getStock));

        //查询商品状态
        List<PilePurchase> pilePurchases = pilePurchaseByCustomerId.stream().filter(p ->
                p.getGoodsNum() > 0
                        && goodsInfoStockMap.containsKey(p.getGoodsInfoId())
                        && goodsInfoStockMap.getOrDefault(p.getGoodsInfoId(), BigDecimal.ZERO).compareTo(BigDecimal.valueOf(p.getGoodsNum())) >= 0
        ).collect(Collectors.toList());

        return CollectionUtils.isNotEmpty(pilePurchases) ? pilePurchases : Lists.newArrayList();
    }
