...
//        }
    ...
    private void updateSkuListSubStock(List<TradeItem> tradeItems,Long wareId, SaleType saleType, Trade trade) {
        ...
        goodsInfoProvider.batchMinusStock(GoodsInfoBatchMinusStockRequest.builder().stockList(stockList).wareId(wareId)
                //线上支付订单 且 不为囤货提货单、预售单时，需要更新锁定库存
                .needUpdateLockStock(!(tradeService.isPayByOfflineOrIsOPK(trade) || trade.getPresellFlag()))
                .build());
...
//        });
    ...
    */
    ...
    public long countVillageTradeByUserAndProvinceId(String customerId,Long provinceId,Long townId,String consigneeId,Long mallMarketId){
        return tradeService.countVillageTradeByUserAndProvinceId(customerId,provinceId,townId,consigneeId,mallMarketId);
    }

    public boolean isHunan(Long provinceId){
        boolean isTrue = provinceId.compareTo(430000L) == 0;
        if(isTrue){
            boolean isOpen = switchProvider.switchIsOpen(SwitchEnum.hunanFirstVillageAdd.getSwitchCode()).getContext();
            return !isOpen;
        }
        return false;
    }
