...
public class LocalTmsOrderService extends TradeTmsService{
    ...
    private static TmsOrderTradeStatusGoodsVO getTmsOrderTradeStatusGoodsVO(TradeItem item) {
        ...
        if(item.getGoodsCubage()!=null)
            goodsVO.setVolumn(item.getGoodsCubage().doubleValue());
    ...
    public void computerTradeDeliveryPrice(List<Trade> tradesDeliveryToStore) {
        ...
        List<Long> storeIdList= new ArrayList<>(tradesDeliveryToStore.size());
        for(Trade trade:tradesDeliveryToStore){
            ...
            storeIdList.add(trade.getSupplier().getStoreId());
            tradeListSameMarket.add(trade);
        }
        Consignee consignee = tradesDeliveryToStore.get(0).getConsignee();
        String buyerId = tradesDeliveryToStore.get(0).getBuyer().getId();
        resetConsigneeById(consignee);
        boolean villageFlag = isToStoreVillageFlag(consignee);
        List<StoreSimpleResponse> simpleResponseList = storeQueryProvider.listSimple(StoreQueryRequest.builder().storeIds(storeIdList).build()).getContext();
        for(Map.Entry<Long, List<Trade>> listEntry:TradeListByMarketIdMap.entrySet()){
            ...
            for(Trade trade:listEntry.getValue()){
                ...
                BigDecimal totalVolumn = getTradeSkuVolumn(trade.getTradeItems(), trade.getGifts());*/
                ...
                tmsOrderAreaStoreVO.setGoodsInfoList(getTmsGoodsInfoVoList(trade.getTradeItems(),trade.getGifts()));
                tmsOrderAreaStoreVO.setQuantity(goodsTotalNum);
                StoreSimpleResponse storeSimpleResponse = simpleResponseList.stream().filter(p->p.getStoreId().compareTo(trade.getSupplier().getStoreId())==0).findFirst().orElse(null);
                tmsOrderAreaStoreVO.setProvinceCode(Objects.toString(storeSimpleResponse.getProvinceId()));
                areaStoreList.add(tmsOrderAreaStoreVO);
            }
            log.warn("提交订单配送到店批发市场[{}],商家明细:[{}]",listEntry.getKey(),JSONObject.toJSONString(areaStoreList));
            List<TmsOrderAmountDTO> orderAmountDTOList = this.calcTradeFreightDeliveryToStore(consignee,areaStoreList,villageFlag,buyerId,listEntry.getKey());
            Trade villageAddTrade = null;
            BigDecimal marketVillageAddliveryPrice = BigDecimal.ZERO;
            for(Trade trade:listEntry.getValue()){
                ...
                if(tmsOrderAmountDTO!=null){
                    ...
                    if(tmsOrderAmountDTO.getIncrease()!=null&&tmsOrderAmountDTO.getIncrease()>0){
                        marketVillageAddliveryPrice=BigDecimal.valueOf(tmsOrderAmountDTO.getIncrease());
                        villageAddTrade=trade;
                    }
                }
                // 8.5.订单总价、原始金额追加运费
            ...
            if(villageAddTrade!=null){
                //villageAddTrade.getTradePrice().setDeliveryPrice(villageAddTrade.getTradePrice().getDeliveryPrice().subtract(marketVillageAddliveryPrice));
                if(verifyService.isHunan(consignee.getProvinceId())){
                    villageAddTrade.getTradePrice().setVillageAddliveryPrice(BigDecimal.ZERO);
                }else {
                    villageAddTrade.getTradePrice().setVillageAddliveryPrice(marketVillageAddliveryPrice);
                }
            }
        }
    }

    private List<TmsOrderAmountDTO> calcTradeFreightDeliveryToStore(Consignee consignee,List<TmsOrderAreaStoreVO> areaStoreList,boolean villageFlag,String buyerId,Long mallMarketId) {
        TmsOrderAreaVO tmsOrderAreaVO =getTmsOrderAreaVO(consignee);
        tmsOrderAreaVO.setAreaStoreList(areaStoreList);
        boolean firstPlusFlag = false;
        if(villageFlag) {
            if(!verifyService.isHunan(consignee.getProvinceId()) ) {
                firstPlusFlag=true;
                long villageTradeCount = verifyService.countVillageTradeByUserAndProvinceId(buyerId, consignee.getProvinceId(),consignee.getTwonId(),consignee.getId(), mallMarketId);
                if (villageTradeCount > 0) {
                    firstPlusFlag=false;
                    villageFlag=false;
                    log.info("用户[{}]收货人乡镇[{}]批发市场[{}]当天已经存在乡镇件加收", buyerId, consignee.getTwonId(), mallMarketId);
                }else{
                    log.info("用户[{}]收货人乡镇[{}]批发市场[{}]当天首单乡镇件加收", buyerId, consignee.getTwonId(), mallMarketId);
                }
            }else{
                log.info("湖南省用户[{}]收货人乡镇[{}]批发市场[{}]乡镇件加收", buyerId, consignee.getTwonId(), mallMarketId);
            }
        }
        tmsOrderAreaVO.setReceiverId(buyerId);
        tmsOrderAreaVO.setFirstPlusFlag(firstPlusFlag);
    ...
    public List<TradeFreightResponse> getFreightForDelivery(List<TradeParams> tradeParamsList) {
        ...
        List<Long> storeIdList= new ArrayList<>(tradeParamsList.size());
        for(TradeParams tradeParams:tradeParamsList){
            ...
            storeIdList.add(tradeParams.getSupplier().getStoreId());
            tradeParamsSameMarket.add(tradeParams);
        }
        Consignee consignee = tradeParamsList.get(0).getConsignee();
        String buyerId = tradeParamsList.get(0).getBuyerId();
        resetConsigneeById(consignee);
        boolean villageFlag = isToStoreVillageFlag(consignee);
        List<StoreSimpleResponse> simpleResponseList = storeQueryProvider.listSimple(StoreQueryRequest.builder().storeIds(storeIdList).build()).getContext();
        for(Map.Entry<Long, List<TradeParams>> listEntry:tradeParamsListByMarketId.entrySet()){
            ...
            for(TradeParams tradeParams:listEntry.getValue()){
                ...
                BigDecimal totalVolumn = getTradeSkuVolumn(tradeParams.getOldTradeItems(), tradeParams.getOldGifts());*/
                TmsOrderAreaStoreVO tmsOrderAreaStoreVO = new TmsOrderAreaStoreVO();
                tmsOrderAreaStoreVO.setStoreId(Objects.toString(tradeParams.getSupplier().getStoreId()));
                tmsOrderAreaStoreVO.setQuantity(goodsTotalNum);
                tmsOrderAreaStoreVO.setGoodsInfoList(getTmsGoodsInfoVoList(tradeParams.getOldTradeItems(),tradeParams.getOldGifts()));
                StoreSimpleResponse storeSimpleResponse = simpleResponseList.stream().filter(p->p.getStoreId().compareTo(tradeParams.getSupplier().getStoreId())==0).findFirst().orElse(null);
                tmsOrderAreaStoreVO.setProvinceCode(Objects.toString(storeSimpleResponse.getProvinceId()));
                areaStoreList.add(tmsOrderAreaStoreVO);
            }
    ...
    public String getTmsFreightTempDesc(String deliveryAddressId) {
        String apiName = "getTmsFreightTempDesc";
        Consignee consignee =new Consignee();
        consignee.setId(deliveryAddressId);
        resetConsigneeById(consignee);
        TmsOrderAreaVO tmsOrderAreaVO =getTmsOrderAreaVO(consignee);
        log.info(param_to_tms,REMOTE_TMS_ORDER_SERVICE, apiName,tmsOrderAreaVO);
        R<String> rdata= remoteTmsOrderService.getTmsFreightTempDesc(tmsOrderAreaVO);
        log.info(param_from_tms,REMOTE_TMS_ORDER_SERVICE,apiName, JSONObject.toJSONString(rdata));
        checkTmsResponse(rdata);
        return rdata.getData();
    }
