 ...
 */
@FeignClient(value = "${application.order.name}", url="${feign.url.order:#{null}}", contextId = "TradeItemQueryProvider")
public interface TradeItemQueryProvider {
     ...
     */
    @PostMapping("/order/${application.order.version}/trade/item/list-by-customer-id")
     ...
     * @return 订单商品快照列表 {@link TradeItemByCustomerIdResponse}
     */
    @PostMapping("/order/${application.order.version}/trade/item/list-all-by-customer-id")
    BaseResponse<TradeItemByCustomerIdResponse> listAllByCustomerId(@RequestBody @Valid TradeItemByCustomerIdRequest
                                                                         request);

    @PostMapping("/order/${application.order.version}/trade/item/list-stock-out-storeId")
     ...
     * @return 订单商品快照列表 {@link TradeItemByCustomerIdResponse}
     */
    @PostMapping("/order/${application.order.version}/trade/item/item_list-by-customer-id")
