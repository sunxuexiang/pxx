 ...
 */
@FeignClient(value = "${application.order.name}", url="${feign.url.order:#{null}}", contextId = "TradeItemProvider")
public interface TradeItemProvider {
     ...
     * @return 操作结果 {@link BaseResponse}
     */
    @PostMapping("/order/${application.order.version}/retail/trade/item/snapshot")
     ...
     * @return 操作结果 {@link BaseResponse}
     */
    @PostMapping("/order/${application.order.version}/trade/item/take-snapshot")
    ...
    @PostMapping("/order/${application.order.version}/trade/item/stock-zero-out")
