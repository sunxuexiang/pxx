 ...
 * @version: 1.0
 */
@FeignClient(value = "${application.order.name}", url="${feign.url.order:#{null}}", contextId = "VerifyQueryProvider")
public interface VerifyQueryProvider {
     ...
     * @return  {@link VerifyGoodsResponse}
     */
    @PostMapping("/order/${application.order.version}/trade/devanning-verify-goods")
     ...
     * @return
     */
    @PostMapping("/order/${application.order.version}/trade/verify-take-goods")
