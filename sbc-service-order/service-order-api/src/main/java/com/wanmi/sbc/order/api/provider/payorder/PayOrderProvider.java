 ...
 * Date: 2018-12-5
 */
@FeignClient(value = "${application.order.name}", url="${feign.url.order:#{null}}", contextId = "PayOrderProvider")
public interface PayOrderProvider {
     ...
     * @return   {@link BaseResponse}
     */
    @PostMapping("/order/${application.order.version}/payorder/generate-new-payorder")
     ...
     * @return   {@link BaseResponse}
     */
    @PostMapping("/order/${application.order.version}/payorder/generate-new-payorder-new-pile")
