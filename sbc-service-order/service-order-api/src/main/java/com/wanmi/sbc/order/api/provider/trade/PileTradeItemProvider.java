 ...
 */
@FeignClient(value = "${application.order.name}", url="${feign.url.order:#{null}}", contextId = "PileTradeItemProvider")
public interface PileTradeItemProvider {
     ...
     */
    @PostMapping("/order/${application.order.version}/pileTrade/item/snapshot")
     ...
     * @return 操作结果 {@link BaseResponse}
     */
    @PostMapping("/order/${application.order.version}/pileTrade/item/delete-by-customer-id")
    BaseResponse deleteByCustomerId(@RequestBody @Valid TradeItemDeleteByCustomerIdRequest request);

    @PostMapping("/order/${application.order.version}/pileTrade/item/stock-zero-out")
