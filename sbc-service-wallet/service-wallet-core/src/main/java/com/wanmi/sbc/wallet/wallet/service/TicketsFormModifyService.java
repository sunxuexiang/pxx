...
import com.wanmi.sbc.wallet.wallet.request.TicketsFormPageRequest;
...
import com.wanmi.sbc.wallet.wallet.repository.TicketsFormQueryRepository;
...
//    private OfflineService offlineService;
    ...
    @Autowired
    public CustomerBankCardProvider customerBankCardProvider;
    ...
    private void assign(MicroServicePage<TicketsFormQueryVO> ticketsFormQueryVOS) {
        ...
        List<Long> collect = content.stream().map(TicketsFormQueryVO::getFormId).collect(Collectors.toList());
        List<TicketsFormLogVo> ticketsFormLogVos = ticketsFormLogService.listByBusinessIds(TicketsFormLogRequest.builder().businessIds(collect).build());
        if (org.springframework.util.CollectionUtils.isEmpty(ticketsFormLogVos)) {
            return;
        }
        //审核人员类型: 1客服 2财务
        for (TicketsFormQueryVO ticketsFormQueryVO : content) {
...
//            request.setWalletId(ticketsForm.getWalletId());
            ...
            //保存银行卡
            customerBankCardProvider.bindBankCard(com.wanmi.sbc.account.api.request.wallet.CustomerBankCardRequest
                    .builder()
...
//            }
            ...
            ticketsForm.setTransferDate(ticketsFormAdoptRequest.getTransferDate());
            ticketsForm.setAccountId(ticketsFormAdoptRequest.getAccountId());
            ticketsForm.setBankNo(ticketsFormAdoptRequest.getBankNo());
            //修改提现信息
            ticketsFormRepository.saveAndFlush(ticketsForm);
            ...
            ticketsFormQueryVO.setCustomerWallet(KsBeanUtil.convert(customerWalletVO, CusWalletVO.class));
