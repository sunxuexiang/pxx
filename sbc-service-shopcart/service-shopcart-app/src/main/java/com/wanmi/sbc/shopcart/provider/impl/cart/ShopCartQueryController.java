...
//            newlistdevanning=list2;
    ...
    public BaseResponse<com.wanmi.sbc.shopcart.api.response.purchase.MarketingGroupCardResponse> devanningPurchaseInfoCache(PurchaseListRequest request) {
        ...
        if (CollectionUtils.isEmpty(devanningGoodsInfoVOS)){
            BaseResponse.success(new com.wanmi.sbc.order.api.response.purchase.MarketingGroupCardResponse());
        }
        //设置商品状态 库存 上下架 是否失效 加设置特价商品的价格
        this.setGoodsStaues(devanningGoodsInfoVOS,request);
        com.wanmi.sbc.shopcart.api.response.purchase.MarketingGroupCardResponse response = getMarketingGroupCardResponse(request, customer, purchaseResponse, devanningGoodsInfoVOS);
        log.info(stopWatch.prettyPrint());
        return BaseResponse.success(response);
    }


    private com.wanmi.sbc.shopcart.api.response.purchase.MarketingGroupCardResponse getMarketingGroupCardResponse(PurchaseListRequest request, CustomerVO customer, PurchaseResponse purchaseResponse, List<DevanningGoodsInfoVO> devanningGoodsInfoVOS) {
    ...
    public BaseResponse<MarketingGroupCartMallResponse> devanningPurchaseMallInfoCache(PurchaseListRequest request) {
        ...
        purchaseRequest.setPageNum(request.getPageNum());
        purchaseRequest.setPageSize(request.getPageSize());
        log.info("统计shopCartService.devanningPageListAndCache耗时时间");
        PurchaseResponse purchaseResponse = shopCartService.devanningPageListAndCache(purchaseRequest);

        List<DevanningGoodsInfoVO> devanningGoodsInfoVOS = purchaseResponse.getDevanningGoodsInfoVOS();
        //设置库存及商品状态
        log.info("设置库存及商品状态耗时时间");
        setStockAndGoodsStatus(devanningGoodsInfoVOS,request.getWareId());

        //设置购物车商品是否参与囤货及囤货库存
        log.info("设置购物车商品是否参与囤货及囤货库存耗时时间");
        setPile(devanningGoodsInfoVOS);
        ...
        //设置商品状态 库存 上下架 是否失效 加设置特价商品的价格
        log.info("设置商品状态 库存 上下架 是否失效 加设置特价商品的价格耗时时间");
        this.setGoodsStaues(devanningGoodsInfoVOS,request);

        MarketingGroupCartMallResponse groupCartMallResponse = new MarketingGroupCartMallResponse();
        List<com.wanmi.sbc.shopcart.api.response.purchase.MarketingGroupCardResponse> marketingGroupCardResponseList = new ArrayList<>(20);
        ...
        log.info("循环调用营销耗时时间");
        List<CompletableFuture<com.wanmi.sbc.shopcart.api.response.purchase.MarketingGroupCardResponse>> futureList = new ArrayList<>();
        longListMap.values().forEach(groupedInfoVos->{
            CompletableFuture<com.wanmi.sbc.shopcart.api.response.purchase.MarketingGroupCardResponse> future = CompletableFuture.supplyAsync(() -> {
                //异步线程计算营销业务
                return getMarketingGroupCardResponse(request, customer, purchaseResponse, groupedInfoVos);
            }, shopExecutor);
        ...
        // 获取每个任务的返回结果
        for(CompletableFuture<com.wanmi.sbc.shopcart.api.response.purchase.MarketingGroupCardResponse> future : futureList){
            try {
                com.wanmi.sbc.shopcart.api.response.purchase.MarketingGroupCardResponse groupCardResponse = future.get();
    ...
    private void calcNoJoinMarketTotalPrice(List<DevanningGoodsInfoVO> noJoinMarketingGoods, com.wanmi.sbc.marketing.api.response.market.MarketingGroupCardResponse marketingGroupCardResponse) {
        //计算未参加营销总价
...
//        });
    ...
    private List<Long> getValidStoreIds(List<DevanningGoodsInfoVO> devanningGoodsInfoVOS) {
        ...
        if(CollectionUtils.isNotEmpty(goodsStoreIds)) {
            List<StoreSimpleResponse> storeVOList = storeQueryProvider.listSimple(StoreQueryRequest.builder().storeIds(goodsStoreIds).build()).getContext();
            List<Long> validStoreIds = storeVOList.stream().filter(s -> s.getDelFlag().toValue() == DeleteFlag.NO.toValue() && s.getStoreState().toValue() == StoreState.OPENING.toValue()).map(StoreSimpleResponse::getStoreId).distinct().collect(Collectors.toList());
