...
import com.wanmi.sbc.shopcart.api.request.purchase.PurchaseMergeRequest;
...
//                reShopCart.setIsCheck(DefaultFlag.YES);
            ...
            cartProducerService.sendMQForOrderRetailShopCar(KsBeanUtil.convert(reShopCart,RetailShopCartVo.class));
    ...
    public void deleteWithCache(RetailShopCartRequest request) {
        ...
        list.stream().forEach(v->{
            ...
            if (redisCache.HashHasKey(key,v.getGoodsInfoId())){
                ...
                v.setIsDelFlag(true);
                cartProducerService.sendMQForOrderRetailShopCar(KsBeanUtil.convert(v,RetailShopCartVo.class));
...
//                }
    ...
    public void batchSaveAndCache(RetailShopCartRequest request) {
        ...
        goodsInfoList.forEach(goods1->{
            ...
            //发送rabbitmq添加mysql
             cartProducerService.sendMQForOrderRetailShopCar(KsBeanUtil.convert(retailShopCart,RetailShopCartVo.class));
...
//            }
    ...
    public void delete(RetailShopCartRequest request) {
        ...
        //删除购物车数据
        String key = RedisKeyConstants.RTAIL_SHOPPING_CART_EXTRA_HASH.concat(request.getCustomerId());
        String numkey = RedisKeyConstants.RTAIL_SHOPPING_CART_HASH.concat(request.getCustomerId());
        request.getGoodsInfoIds().forEach(v -> {
