...
public class GoodsInfoQueryRequest extends BaseQueryRequest implements Serializable {
    ...
    /**
     * 白鲸散批图片查询条件标志(BOSS运营后台专用)
     */
    private Boolean retailImgFlag = Boolean.FALSE;
    ...
    public Specification<GoodsInfo> getWhereCriteria() {
        return (root, cquery, cbuild) -> {
            ...
            //映射表伪spuId
            if(CollectionUtils.isNotEmpty(parentGoodsInfoIds)){
                predicates.add(root.get("parentGoodsInfoId").in(parentGoodsInfoIds));
            }
            //批量SKU编号
            ...
            //erp编码集合
            if(CollectionUtils.isNotEmpty(erpGoodsInfoNoList)){
                predicates.add(root.get("erpGoodsInfoNo").in(erpGoodsInfoNoList));
            }
            ...
            // 不等于的商家类型
            if(CollectionUtils.isNotEmpty(notEqCompanyTypes)){
                predicates.add(cbuild.not(root.get("companyType").in(notEqCompanyTypes)));
            }
             ...
             * erp编码筛选
             */
            if (StringUtils.isNotEmpty(erpGoodsInfoNo)) {
                predicates.add(cbuild.equal(root.get("erpGoodsInfoNo"),erpGoodsInfoNo));
            }

            /**
             * 是否指定区域销售
             */
            if (Objects.nonNull(areaFlag)) {
                predicates.add(cbuild.equal(root.get("areaFlag"), areaFlag));
            }

            //条形码
            if(StringUtils.isNotBlank(likeGoodsInfoBarcode)){
                predicates.add(cbuild.like(root.get("goodsInfoBarcode"), StringUtil.SQL_LIKE_CHAR.concat(XssUtils.replaceLikeWildcard(likeGoodsInfoBarcode.trim())).concat(StringUtil.SQL_LIKE_CHAR)));
            }
    ...
    public Specification<RetailGoodsInfo> getRetailWhereCriteria() {
        return (root, cquery, cbuild) -> {
            ...
            if (retailImgFlag) {
                predicates.add(cbuild.isNull(cbuild.lower(root.get("goodsInfoImg"))));
            }
     ...
     * @return
     */
    public Specification<BulkGoodsInfo> getBulkWhereCriteria() {
        return (root, cquery, cbuild) -> {
            List<Predicate> predicates = new ArrayList<>();
