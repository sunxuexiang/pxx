...
@Slf4j
@Service
public class RetailGoodsService {


    @Autowired
    RetailGoodsInfoRepository retailGoodsInfoRepository;
    @Autowired
    ...
    @Autowired
    private RetailGoodsRepository goodsRepository;
    ...
    @Autowired
    private RetailGoodsInfoRepository goodsInfoRepository;
     ...
     * @return list
     */
    public RetailGoodsQueryResponse page(GoodsQueryRequest request) {
        RetailGoodsQueryResponse response = new RetailGoodsQueryResponse();

        List<RetailGoodsInfo> goodsInfos = new ArrayList<>();
        ...
        if (checkSKU) {
            ...
            List<RetailGoodsInfo> infos = retailGoodsInfoRepository.findAll(infoQueryRequest.getRetailWhereCriteria());
            if (CollectionUtils.isNotEmpty(infos)) {
                request.setGoodsIds(infos.stream().map(RetailGoodsInfo::getGoodsId).collect(Collectors.toList()));
            } else {
                return response;
            }
        }
        ...
        if (Objects.isNull(request.getStockUp())) {
            Page<RetailGoods> goodsPage = goodsRepository.findAll(request.getRetailWhereCriteria(), request.getPageRequest());
            if (CollectionUtils.isNotEmpty(goodsPage.getContent())) {
                List<String> goodsIds = goodsPage.getContent().stream().map(RetailGoods::getGoodsId).collect(Collectors.toList());
                //查询所有SKU
                infoQueryRequest.setLikeGoodsInfoNo(null);
                infoQueryRequest.setGoodsIds(goodsIds);
                infoQueryRequest.setDelFlag(DeleteFlag.NO.toValue());
                goodsInfos.addAll(retailGoodsInfoRepository.findAll(infoQueryRequest.getRetailWhereCriteria()));
                ...
                goodsInfos.forEach(goodsInfo -> {
                    ...
                    if (StringUtils.isBlank(goodsInfo.getGoodsInfoImg())) {
                        goodsInfo.setGoodsInfoImg(goodsPage.getContent().stream().filter(goods -> goods.getGoodsId().equals(goodsInfo.getGoodsId())).findFirst().orElse(new RetailGoods()).getGoodsImg());
                    }
                ...
                goodsPage.getContent().forEach(goods -> {
                    goods.setGoodsInfoIds(goodsInfos.stream().filter(goodsInfo -> goodsInfo.getGoodsId().equals(goods.getGoodsId())).map(RetailGoodsInfo::getGoodsInfoId).collect(Collectors.toList()));
                    //合计库存
                    goods.setStock(goodsInfos.stream().filter(goodsInfo -> goodsInfo.getGoodsId().equals(goods.getGoodsId()) && Objects.nonNull(goodsInfo.getStock())).mapToLong(RetailGoodsInfo::getStock).sum());
                    //取SKU最小市场价
                    goods.setMarketPrice(goodsInfos.stream().filter(goodsInfo -> goods.getGoodsId().equals(goodsInfo.getGoodsId())).filter(goodsInfo -> Objects.nonNull(goodsInfo.getMarketPrice())).map(RetailGoodsInfo::getMarketPrice).min(BigDecimal::compareTo).orElse(goods.getMarketPrice()));
                    //取SKU最小供货价
                    goods.setSupplyPrice(goodsInfos.stream().filter(goodsInfo -> goods.getGoodsId().equals(goodsInfo.getGoodsId())).filter(goodsInfo -> Objects.nonNull(goodsInfo.getSupplyPrice())).map(RetailGoodsInfo::getSupplyPrice).min(BigDecimal::compareTo).orElse(goods.getSupplyPrice()));
                    //取商品类型是否特价
                    ...
                    //取商品特价价格
                    goods.setSpecialPrice(goodsInfos.stream().filter(goodsInfo -> goods.getGoodsId().equals(goodsInfo.getGoodsId())).filter(goodsInfo -> Objects.nonNull(goodsInfo.getSpecialPrice())).map(RetailGoodsInfo::getSpecialPrice).min(BigDecimal::compareTo).orElse(goods.getSpecialPrice()));

                    goods.setGoodsInfoBatchNo(goodsInfos.stream().filter(goodsInfo -> goods.getGoodsId().equals(goodsInfo.getGoodsId()) && !Objects.equals(goodsInfo.getGoodsInfoType(), 0)).filter(goodsInfo -> Objects.nonNull(goodsInfo.getGoodsInfoBatchNo())).map(RetailGoodsInfo::getGoodsInfoBatchNo).findFirst().orElse(""));
                    ...
                    goods.setVipPrice(goodsInfos.stream().filter(goodsInfo -> goods.getGoodsId().equals(
                            ...
                            .map(RetailGoodsInfo::getVipPrice).min(BigDecimal::compareTo).orElse(goods.getVipPrice()));
                    goods.setMarketingId(goodsInfos.stream().filter(goodsInfo -> goodsInfo.getGoodsId().equals(goods.getGoodsId()) && Objects.nonNull(goodsInfo.getMarketingId())).map(RetailGoodsInfo::getMarketingId).findFirst().orElse(0L));
                    goods.setPurchaseNum(goodsInfos.stream().filter(goodsInfo -> goodsInfo.getGoodsId().equals(goods.getGoodsId()) && Objects.nonNull(goodsInfo.getPurchaseNum())).map(RetailGoodsInfo::getPurchaseNum).findFirst().orElse(0L));
                });
                ...
                brandRequest.setBrandIds(goodsPage.getContent().stream().filter
                        (goods -> Objects.nonNull(goods.getBrandId())).map(RetailGoods::getBrandId).distinct().collect(Collectors.toList()));
                ...
                cateRequest.setCateIds(goodsPage.getContent().stream().filter(goods -> Objects.nonNull(goods.getCateId())).map(RetailGoods::getCateId).distinct().collect(Collectors.toList()));
            ...
            List<RetailGoods> goodsList = new ArrayList<>();
            Page<RetailGoodsInfo> goodsInfoPage = retailGoodsInfoRepository.findAll(infoQueryRequest.getRetailWhereCriteria(),request.getPageRequest());
            if (CollectionUtils.isNotEmpty(goodsInfoPage.getContent())){
                List<RetailGoodsInfo> goodsInfoList = goodsInfoPage.getContent();
                List<String> goodsIds = goodsInfoPage.stream().map(RetailGoodsInfo::getGoodsId).collect(Collectors.toList());
                ...
                goodsList.addAll(goodsRepository.findAll(goodsQueryRequest.getRetailWhereCriteria()));
                goodsList.stream().forEach(goods -> {
                    goods.setMarketingId(goodsInfoList.stream().filter(goodsInfo -> goodsInfo.getGoodsId().equals(goods.getGoodsId()) && Objects.nonNull(goodsInfo.getMarketingId())).map(RetailGoodsInfo::getMarketingId).findFirst().orElse(0L));
                    goods.setPurchaseNum(goodsInfoList.stream().filter(goodsInfo -> goodsInfo.getGoodsId().equals(goods.getGoodsId()) && Objects.nonNull(goodsInfo.getPurchaseNum())).map(RetailGoodsInfo::getPurchaseNum).findFirst().orElse(0L));
                });
            }
     ...
     * @return list
     */
    public RetailGoodsEditResponse findInfoById(String goodsId, Long wareId, Boolean matchWareHouseFlag) throws SbcRuntimeException {
        RetailGoodsEditResponse response = new RetailGoodsEditResponse();
        RetailGoods goods = goodsRepository.findById(goodsId).orElse(null);
        ...
        infoQueryRequest.setDelFlag(DeleteFlag.NO.toValue());
        List<RetailGoodsInfo> goodsInfos = goodsInfoRepository.findAll(infoQueryRequest.getRetailWhereCriteria());
        ...
        //查询sku库存信息
        List<String> goodsInfoIds = goodsInfos.stream().map(RetailGoodsInfo::getGoodsInfoId).collect(Collectors.toList());
...
//           goodsInfoService.calGoodsInfoStock(goodsInfo,goodsNumsMap);
     ...
     * @return
     */
    public RetailGoods getRetailGoodsById(String goodsId) {
    ...
    public void modifyGoodsSeqNum(String goodsId, Integer goodsSeqNum) {
        //验证商品排序序号是否重复
        if(goodsSeqNum!=null){
            List<RetailGoods> _goodsList = goodsRepository.getExistByGoodsSeqNum(goodsSeqNum, goodsId);
     ...
     * @return
     */
    public List<RetailGoods> listRetailGoodsByGoodsIds(List<String> goodsIds) {
        GoodsQueryRequest goodsQueryRequest = new GoodsQueryRequest();
        goodsQueryRequest.setGoodsIds(goodsIds);

        List<RetailGoods> goodsList = goodsRepository.findAll(goodsQueryRequest.getRetailWhereCriteria());

        for (RetailGoods goods : goodsList) {
...
//                goodsWareStocks = goodsWareStocks.stream().filter(param -> onlineWareIds.contains(param.getWareId())).collect(Collectors.toList());
    ...
    public List<RetailGoods> findAll(GoodsQueryRequest goodsQueryRequest){
        return goodsRepository.findAll(goodsQueryRequest.getRetailWhereCriteria());
    }
...
//
     ...
     * @return
     */
    public RetailGoods getGoodsById(String goodsId) {
        return goodsRepository.findById(goodsId).orElse(null);
    }
     ...
     * @return
     */
    public List<RetailGoods> listByProviderGoodsIds(List<String> goodsIds) {
        GoodsQueryRequest goodsQueryRequest = new GoodsQueryRequest();
     ...
     */
    @Transactional
    public Map<String, Object> edit(RetailGoodsSaveRequest saveRequest) throws SbcRuntimeException {
        RetailGoods newGoods = saveRequest.getGoods();
        RetailGoods oldGoods = goodsRepository.findById(newGoods.getGoodsId()).orElse(null);
        ...
        if (goodsRepository.count(queryRequest.getRetailWhereCriteria()) > 0) {
        ...
        GoodsInfoQueryRequest infoQueryRequest = new GoodsInfoQueryRequest();
        infoQueryRequest.setDelFlag(DeleteFlag.NO.toValue());
        infoQueryRequest.setGoodsInfoNos(saveRequest.getGoodsInfos().stream().map(RetailGoodsInfo::getGoodsInfoNo).distinct().collect(Collectors.toList()));
        ...
        //验证SKU编码重复
        List<RetailGoodsInfo> goodsInfosList = goodsInfoRepository.findAll(infoQueryRequest.getRetailWhereCriteria());
        if (CollectionUtils.isNotEmpty(goodsInfosList)) {
            throw new SbcRuntimeException(GoodsErrorCode.SKU_NO_H_EXIST, new Object[]{StringUtils.join(goodsInfosList.stream().map(RetailGoodsInfo::getGoodsInfoNo).collect(Collectors.toList()), ",")});
        }
        ...
        List<RetailGoodsInfo> newGoodsInfo = new ArrayList<>();//需要被添加的sku信息

        //更新原有的SKU列表
        List<RetailGoodsInfo> goodsInfos = saveRequest.getGoodsInfos();
        List<RetailGoodsInfo> oldGoodsInfos = new ArrayList<>();//需要被更新的sku信息
        ...
        if (CollectionUtils.isNotEmpty(goodsInfos)) {
            ...
            infoQueryRequest.setDelFlag(DeleteFlag.NO.toValue());
            List<RetailGoodsInfo> oldInfos = goodsInfoRepository.findAll(infoQueryRequest.getRetailWhereCriteria());

            if (CollectionUtils.isNotEmpty(oldInfos)) {
                for (RetailGoodsInfo oldInfo : oldInfos) {
                    if (Objects.isNull(oldInfo.getStock())) {
                        oldInfo.setStock(0L);
                    }

                    Optional<RetailGoodsInfo> infoOpt = goodsInfos.stream().filter(goodsInfo -> oldInfo.getGoodsInfoId().equals(goodsInfo.getGoodsInfoId())).findFirst();
                    //如果SKU存在，更新
                ...
                //更新最新库存信息
                List<RetailGoodsInfo> goodsInfoList = goodsInfos.stream().
            ...
            //只保存新SKU
            for (RetailGoodsInfo sku : goodsInfos) {
     ...
     */
    private void saveGoodsPrice(List<RetailGoodsInfo> goodsInfos, RetailGoods goods, RetailGoodsSaveRequest saveRequest) {
        List<String> skuIds = new ArrayList<>();
        //提取非独立设价的Sku编号,进行清理设价数据
        if (goods.getPriceType() == 1 && goods.getAllowPriceSet() == 0) {
            skuIds = goodsInfos.stream()
                    .map(RetailGoodsInfo::getGoodsInfoId)
                    .collect(Collectors.toList());
        } else {
            skuIds = goodsInfos.stream()
    ...
    public List<RetailGoods> findByProviderGoodsId(String providerGoodsId) {
     ...
     */
    @Transactional
    public void synStoreGoodsInfoAndStandardSkuForSupplyPrice(RetailGoodsSaveRequest goodsSaveRequest, List<RetailGoods> providerGoods) {
        //同步商家商品的供货价
        providerGoods.forEach(s->{s.setSupplyPrice(goodsSaveRequest.getGoods().getSupplyPrice());});
        goodsRepository.saveAll(providerGoods);

        List<String> goodIds = providerGoods.stream().map(RetailGoods::getGoodsId).collect(Collectors.toList());
        ...
        List<RetailGoodsInfo> providerGoodsInfos = goodsSaveRequest.getGoodsInfos();
        ...
        List<RetailGoodsInfo> storeGoodsInfos = goodsInfoRepository.findByGoodsIdIn(goodIds);
    ...
    public List<String> synDeleteStoreGoodsInfoAndStandardSku(List<String> delInfoIds) {
        if(delInfoIds != null){
            List<RetailGoodsInfo> byProviderGoodsInfoIdIn = goodsInfoRepository.findByProviderGoodsInfoIdIn(delInfoIds);
            goodsInfoRepository.deleteByProviderGoodsInfoId(delInfoIds);
            standardSkuRepository.deleteByGoodsIds(delInfoIds);
            return byProviderGoodsInfoIdIn.stream().map(RetailGoodsInfo::getGoodsInfoId).collect(Collectors.toList());
        }
     ...
     * @param goods 商品信息
     */
    public void checkBasic(RetailGoods goods) {
    ...
    public List<RetailGoods> listByGoodsIdsNoValid(List<String> goodsIds) {
        GoodsQueryRequest goodsQueryRequest = new GoodsQueryRequest();
        goodsQueryRequest.setGoodsIds(goodsIds);
    ...
    public long countByCondition(GoodsQueryRequest request){
        return goodsRepository.count(request.getRetailWhereCriteria());
    }

    @Transactional(rollbackFor = Exception.class)
    public void batchModifyRetailCate(GoodsBatchModifyCateRequest request) {
