...
@FeignClient(value = "${application.returnOrder.name}", url="${feign.url.returnOrder:#{null}}", contextId = "ReturnPileTradeGroupQueryProvider")
public interface PileTradeGroupQueryProvider {
     ...
     */
    @PostMapping("/returnOrder/${application.order.version}/pile/trade/get-trade-group-by-group-ids")
