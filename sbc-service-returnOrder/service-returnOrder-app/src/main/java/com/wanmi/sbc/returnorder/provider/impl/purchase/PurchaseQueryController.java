package com.wanmi.sbc.returnorder.provider.impl.purchase;
...
public class PurchaseQueryController implements PurchaseQueryProvider {
    ...
    @Autowired
    private MarketingScopeQueryProvider marketingScopeQueryProviderl;
    ...
    public BaseResponse<PurchaseGetGoodsMarketingResponse> getGoodsMarketing(@RequestBody @Valid
        ...
        return BaseResponse.success(purchaseService.getGoodsMarketing(goodsInfos, request.getCustomer(), request.getWareId()));
    }
    ...
    public BaseResponse<PurchaseCountGoodsResponse> countGoods(@RequestBody @Valid PurchaseCountGoodsRequest request) {
        ...
        if(request.getSaasStatus()!=null
            ...
            total = purchaseService.countGoods(request.getCustomerId(), request.getInviteeId());
        }
    ...
    public BaseResponse<ProcurementConfigResponse> queryProcurementConfig(){
        return purchaseService.getProcurementType();
    }
    ...
    public BaseResponse<CheckPurchaseNumResponse> checkPurchaseNum(CheckPurchaseNumRequest request) {
        ...
        List<Long> devannings = request.getCheckPurchaseNumDTOS().stream().map(CheckPurchaseNumDTO::getDevanningId).collect(Collectors.toList());
        List<DevanningGoodsInfoVO> devanningGoodsInfoVOS = devanningGoodsInfoQueryProvider.listDevanningGoodsInfoAndStcokByIds(
                GoodsInfoAndStockListByIdsRequest.builder()
                        .goodsInfoIds(goodsInfoIds)
                        .devanningIds(devannings).build()).getContext().getDevanningGoodsInfoVOS();
        ...
        if(CollectionUtils.isNotEmpty(devanningGoodsInfoVOS)){
            devanningGoodsInfoVOS.forEach(goodsInfoVO -> {
                if(Objects.nonNull(goodsInfoVO.getPurchaseNum())){
                    CheckPurchaseNumDTO checkPurchaseNumDTO1 = checkPurchaseNumDTOS.stream().
                    ...
                    BigDecimal num = checkPurchaseNumDTO1.getDivisorFlag().multiply(BigDecimal.valueOf(checkPurchaseNumDTO1.getNum()));

                    if(goodsInfoVO.getPurchaseNum() >= 0 && num.compareTo(BigDecimal.valueOf(goodsInfoVO.getPurchaseNum())) > 0){
                        ...
                        vo.setDevanningId(goodsInfoVO.getDevanningId());
                        vo.setDivisorFlag(goodsInfoVO.getDivisorFlag());
    ...
    public BaseResponse<PurchaseListResponse> purchaseInfo(@RequestBody @Valid PurchaseListRequest request) {
        ...
        try {
            ...
            if (request.getIsPage()) {
                ...
                purchaseResponse = purchaseService.pageList(purchaseRequest);
            } else {
                purchaseResponse = purchaseService.list(purchaseRequest);
            }
            sb.append(",purchaseService.list end time=");
...
//                }
                ...
                if (!request.getIsPage()) {
                    // 获取采购营销信息及同步商品营销
                    PurchaseMarketingResponse purchasesMarketing = purchaseService.getPurchasesMarketingInit(intersectionList
                            , requestGoods, customer, request.getWareId());

                    sb.append(",purchaseService.getPurchasesMarketing end time=");
                    sb.append(System.currentTimeMillis() - sTm);
                    sTm = System.currentTimeMillis();

                    response.setGoodsMarketingMap(purchasesMarketing.getMap());
                    response.setSelfBuying(false);
                    //设置大客户价 新增大客户标识判断 add by jiangxin 20211203 vip大客户价格商品不参与营销
...
//                        log.info("购物车不分页，满足设置大客户价条件==============================》customerAccount={}",customer.getCustomerAccount());
...
//                                    vo.setMarketPrice(null != goodsInfoVO.getVipPrice() && goodsInfoVO.getVipPrice().compareTo(BigDecimal.ZERO) > 0 ? goodsInfoVO.getVipPrice() : goodsInfoVO.getMarketPrice());
                    ...
                    sb.append(",purchaseService.calcAmount start time=");
                    sb.append(System.currentTimeMillis() - sTm);
                    sTm = System.currentTimeMillis();

                    response = purchaseService.calcAmount(KsBeanUtil.convert(response, PurchaseListResponse.class), customer, intersectionList);

                    sb.append(",purchaseService.calcAmount end time=");
                    sb.append(System.currentTimeMillis() - sTm);
                    sTm = System.currentTimeMillis();
                } else {
                    //特价判断，设置特价
                    ...
                    if (request.getIsRefresh()) {
                        PurchaseMarketingResponse purchasesMarketing = purchaseService.getPurchasesMarketingInit(intersectionList,
                                requestGoods, customer, request.getWareId());
                        // 获取采购单商品参与营销信息的集合
//                        Map<String,GoodsInfoVO> marketingMap = purchasesMarketing.getGoodsInfos().stream().collect(Collectors.toMap(GoodsInfoVO::getGoodsInfoId,g -> g, (k1,k2) -> k1));
                        ...
                        goodsInfos = response.getGoodsInfos().stream().map(goodsInfo ->
                                purchasesMarketing.getGoodsInfos().stream()
                                    ...
                                    if ((null != customer.getEnterpriseStatusXyy() && customer.getEnterpriseStatusXyy().equals(EnterpriseCheckState.CHECKED))
                                            || (Objects.nonNull(customer.getVipFlag()) && DefaultFlag.YES.equals(customer.getVipFlag()))) {
                                        log.info("参与营销活动，满足设置大客户价条件==============================》vipPrice={}",goodsInfoVO.getVipPrice());
                                        GoodsInfoVO vo = new GoodsInfoVO();
                                        KsBeanUtil.copyPropertiesThird(goodsInfoVO, vo);
//                                        vo.setMarketPrice(null != goodsInfoVO.getVipPrice() && goodsInfoVO.getVipPrice().compareTo(BigDecimal.ZERO) > 0 ? goodsInfoVO.getVipPrice() : goodsInfoVO.getMarketPrice());
                                        vo.setMarketPrice(goodsInfoVO.getMarketPrice());
    ...
    public BaseResponse<PurchaseListResponse> newPurchaseInfo(@RequestBody @Valid PurchaseListNewRequest request) {
        ...
        try {
            ...
            if (CollectionUtils.isNotEmpty(response.getGoodsInfos())) {
                ...
                if (!request.getIsPage()) {
                    // 获取采购营销信息及同步商品营销
                    PurchaseMarketingResponse purchasesMarketing = purchaseService.getPurchasesMarketingInit(request.getGoodsInfoIds(), requestGoods, customer, request.getWareId());
                    ...
                    response = purchaseService.calcAmount(KsBeanUtil.convert(response, PurchaseListResponse.class), customer, request.getGoodsInfoIds());
...
//                        return Long.compare(su2, su);
    ...
    public BaseResponse<PurchaseStoreMarketingResponse> getStoreMarketings(@RequestBody @Valid PurchaseStoreMarketingRequest request) {
        ...
        try {
            ...
            purchaseService.updateIsCheck(purchaseRequest, request.getGoodsInfoIdList());

            sb.append(",purchaseService.updateIsCheck end time=");
            sb.append(System.currentTimeMillis()-sTm);
            sTm = System.currentTimeMillis();
...
//            Map<Long, List<PurchaseMarketingCalcResponse>> storeMarketing = purchaseService.getStoreMarketingBase(request.getGoodsMarketingDTOList(), request.getCustomer(),
//                    new PurchaseFrontRequest(), reqGoodsInfoIds, request.getWareId(), reqGoodsInfoFilter);
            Map<Long, List<PurchaseMarketingCalcResponse>> storeMarketing = purchaseService.getStoreMarketingBaseCopy(request.getGoodsMarketingDTOList(), request.getCustomer(),
...
//                }
//            }
            sb.append(",purchaseService.getStoreMarketingBase end time=");
            sb.append(System.currentTimeMillis()-sTm);
            sTm = System.currentTimeMillis();
            ...
            // 获取采购营销信息及同步商品营销
//        PurchaseMarketingResponse purchasesMarketing = purchaseService.getPurchasesMarketing(request.getGoodsInfoIdList(), reqGoodsInfo, request.getCustomer(), request.getWareId());
            ...
            // 计算价格
            purchaseListResponse = purchaseService.calcAmount(purchaseListResponse, request.getCustomer(), request.getGoodsInfoIdList());
