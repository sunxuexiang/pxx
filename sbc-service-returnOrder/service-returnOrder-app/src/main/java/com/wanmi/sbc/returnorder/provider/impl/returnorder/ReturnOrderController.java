...
public class ReturnOrderController implements ReturnOrderProvider {
    ...
    public BaseResponse<ReturnOrderAddResponse> add(@RequestBody @Valid ReturnOrderAddRequest request) {
        ...
        BoolFlag batchFlag = request.getReturnOrder().getBatchFlag();
        if (Objects.nonNull(batchFlag) && Objects.equals(batchFlag, BoolFlag.YES)) {
            ReturnOrderDTO returnOrder = request.getReturnOrder();
            ReturnOrderLogistics logistics = new ReturnOrderLogistics();
            logistics.setTid(returnOrder.getTid());
            logistics.setRid(rid);
            logistics.setStoreId(returnOrder.getCompany().getStoreId());
            logistics.setDescAddr(returnOrder.getDescAddr());
            logistics.setPickAddr(returnOrder.getPickAddr());
            logistics.setPickPhone(returnOrder.getPickPhone());
            logistics.setCompanyName(returnOrder.getCompanyName());
            logistics.setCustomerId(request.getOperator().getUserId());
            logistics.setCreateTime(LocalDateTime.now());
            returnOrderLogisticsRepository.saveAndFlush(logistics);
        }
