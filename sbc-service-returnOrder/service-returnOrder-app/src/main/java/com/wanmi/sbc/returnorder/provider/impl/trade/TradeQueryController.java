...
import com.wanmi.sbc.returnorder.bean.constant.ConstantContent;
...
import com.wanmi.sbc.returnorder.trade.reponse.TradeLogisticsReponse;
...
//        }
    ...
    private void setTradeVOLogisticsByWith100(com.wanmi.sbc.returnorder.bean.vo.TradeVO tradeVo) {
        if (CollectionUtils.isEmpty(tradeVo.getTradeDelivers())) {
            log.info("发货记录为空{}", tradeVo.getId());
            return;
        }
        for (com.wanmi.sbc.returnorder.bean.vo.TradeDeliverVO tradeDeliver : tradeVo.getTradeDelivers()) {
            com.wanmi.sbc.returnorder.bean.vo.LogisticsVO tradeDeliverLogistics = tradeDeliver.getLogistics();
    ...
    public BaseResponse<TradeGetByIdResponse> getByIdManager(@RequestBody @Valid TradeGetByIdManagerRequest request) {
        com.wanmi.sbc.returnorder.bean.vo.TradeVO trade = tradeService.detailVO(request.getTid());
        List<ProviderTrade> providerTrades = providerTradeService.queryAll(ProviderTradeQueryRequest.builder().parentId(trade.getId()).build());
        log.info("打印原始数据中的数量jeffrey: {}", JSON.toJSONString(trade));
        //求出商品总件数
        Long allNum = 0L;
        for (com.wanmi.sbc.returnorder.bean.vo.TradeItemVO tradeItem : trade.getTradeItems()) {
            Long num = tradeItem.getNum() == null ? 0L : tradeItem.getNum();
            allNum += num;
        }
        if (trade.getGoodsTotalNum() == null) {
        ...
        List<TradeVO> providerTradeVOS = KsBeanUtil.convert(providerTrades, TradeVO.class);
        BaseResponse<TradeGetByIdResponse> baseResponse = BaseResponse.success(TradeGetByIdResponse.builder().
                tradeVO(KsBeanUtil.convert(trade, com.wanmi.sbc.returnorder.bean.vo.TradeVO.class)).build());
        //赋值是否标星
        ...
        Map<String, String> collect = trade.getTradeItems().stream().collect(Collectors.toMap(TradeItemVO::getSkuId, TradeItemVO::getGoodsSubtitle, (a, b) -> a));
...
//    }
    ...
    @Override
    public BaseResponse<List<OrderSalesRankingSort>> orderSort() {
        return BaseResponse.success(tradeService.orderSort());
    }
    @Override
