...
import com.wanmi.sbc.returnorder.api.provider.purchase.PurchaseProvider;
...
import com.wanmi.sbc.returnorder.shopcart.ShopCart;
import com.wanmi.sbc.returnorder.shopcart.ShopCartNewPileTradeService;
...
@RestController
@Log4j2
public class ShopCartQueryController implements ShopCartQueryProvider {
    ...
    public BaseResponse<PurchaseQueryResponse> queryShopCarExit(PurchaseQueryCacheRequest request) {
        List<ShopCart> shopCartList = shopCartService.getShopCarCache(request.getCustomerId(), request.getWareId().toString(), KsBeanUtil.convert(request.getDevaningId(), String.class));
        PurchaseQueryResponse purchaseQueryResponse = new PurchaseQueryResponse();
        purchaseQueryResponse.setPurchaseList(KsBeanUtil.convertList(shopCartList, PurchaseVO.class));
        return BaseResponse.success(purchaseQueryResponse);
    }
...
//            newlistdevanning=list2;
            ...
            sb.append("com.wanmi.sbc.returnorder.provider.impl.shopcart.ShopCartQueryController.devanningPurchaseInfo end time=");
    ...
    @Override
    public BaseResponse<com.wanmi.sbc.returnorder.api.response.purchase.MarketingGroupCardResponse> devanningPurchaseInfoCache(PurchaseListRequest request) {
        ...
        //设置库存及商品状态
        setStockAndGoodsStatus(devanningGoodsInfoVOS,request.getWareId());
        //设置购物车商品是否参与囤货及囤货库存
        setPile(devanningGoodsInfoVOS);

        if (CollectionUtils.isEmpty(devanningGoodsInfoVOS)){
            BaseResponse.success(new com.wanmi.sbc.returnorder.api.response.purchase.MarketingGroupCardResponse());
        }
        //设置商品状态 库存 上下架 是否失效 加设置特价商品的价格
        this.setGoodsStaues(devanningGoodsInfoVOS,request);
        com.wanmi.sbc.returnorder.api.response.purchase.MarketingGroupCardResponse response = getMarketingGroupCardResponse(request, stopWatch, customer, purchaseResponse, devanningGoodsInfoVOS);
        log.info(stopWatch.prettyPrint());
        return BaseResponse.success(response);
    }


    private com.wanmi.sbc.returnorder.api.response.purchase.MarketingGroupCardResponse getMarketingGroupCardResponse(PurchaseListRequest request, StopWatch stopWatch, CustomerVO customer, PurchaseResponse purchaseResponse, List<DevanningGoodsInfoVO> devanningGoodsInfoVOS) {
        ...
        Long goodsStoreId =  devanningGoodsInfoVOS.get(0).getStoreId();
        List<CompanyMallBulkMarketVO> mallBulkMarketVOS = companyIntoPlatformQueryProvider.listMarketByStoreId(goodsStoreId).getContext();
        Long mallBulkMarketId;
        String mallBulkMarketName;
        if(CollectionUtils.isNotEmpty(mallBulkMarketVOS)){
            mallBulkMarketId = mallBulkMarketVOS.get(0).getMarketId();
            mallBulkMarketName = mallBulkMarketVOS.get(0).getMarketName();
        } else {
            mallBulkMarketName = null;
            mallBulkMarketId = null;
        }
        devanningGoodsInfoVOS.forEach(v->{
            v.setMallBulkMarketId(mallBulkMarketId);
            v.setMallBulkMarketName(mallBulkMarketName);
        ...
        if (CollectionUtils.isNotEmpty(noJoinMarketingGoods)){
            ...
            calcNoJoinMarketTotalPrice(noJoinMarketingGoods, marketingGroupCardResponse);
            //没参加营销的商品
        ...
        stopWatch.stop();
        com.wanmi.sbc.returnorder.api.response.purchase.MarketingGroupCardResponse response = KsBeanUtil.convert(marketingGroupCardResponse, com.wanmi.sbc.returnorder.api.response.purchase.MarketingGroupCardResponse.class);
        response.setStoreId(devanningGoodsInfoVOS.get(0).getStoreId());
        response.setStoreName(devanningGoodsInfoVOS.get(0).getStoreName());
        response.setMallBulkMarketId(devanningGoodsInfoVOS.get(0).getMallBulkMarketId());
        response.setMallBulkMarketName(devanningGoodsInfoVOS.get(0).getMallBulkMarketName());
    ...
    @Override
    public BaseResponse<MarketingGroupCartMallResponse> devanningPurchaseMallInfoCache(PurchaseListRequest request) {
        StopWatch stopWatch = new StopWatch();
        ...
        List<DevanningGoodsInfoVO> devanningGoodsInfoVOS = purchaseResponse.getDevanningGoodsInfoVOS();
        //设置库存及商品状态
        stopWatch.start("设置库存及商品状态耗时时间");
        setStockAndGoodsStatus(devanningGoodsInfoVOS,request.getWareId());
        stopWatch.stop();

        //设置购物车商品是否参与囤货及囤货库存
        stopWatch.start("设置购物车商品是否参与囤货及囤货库存耗时时间");
        setPile(devanningGoodsInfoVOS);
        stopWatch.stop();
        ...
        //设置商品状态 库存 上下架 是否失效 加设置特价商品的价格
        stopWatch.start("设置商品状态 库存 上下架 是否失效 加设置特价商品的价格耗时时间");
        this.setGoodsStaues(devanningGoodsInfoVOS,request);
        stopWatch.stop();

        MarketingGroupCartMallResponse groupCartMallResponse = new MarketingGroupCartMallResponse();
        List<com.wanmi.sbc.returnorder.api.response.purchase.MarketingGroupCardResponse> marketingGroupCardResponseList = new ArrayList<>(20);

        Map<Long,List<DevanningGoodsInfoVO>> longListMap = IteratorUtils.groupBy(devanningGoodsInfoVOS,DevanningGoodsInfoVO::getStoreId);

        stopWatch.start("循环调用营销耗时时间");
//        longListMap.values().forEach(groupedInfoVos->{
//            com.wanmi.sbc.returnorder.api.response.purchase.MarketingGroupCardResponse response = getMarketingGroupCardResponse(request, stopWatch, customer, purchaseResponse, groupedInfoVos);
//            marketingGroupCardResponseList.add(response);
//        });

        //多线程处理营销关联业务计算,提升购物车查询性能 @jkp 2023/8/2
        List<CompletableFuture<com.wanmi.sbc.returnorder.api.response.purchase.MarketingGroupCardResponse>> futureList = new ArrayList<>();
        longListMap.values().forEach(groupedInfoVos->{
            CompletableFuture<com.wanmi.sbc.returnorder.api.response.purchase.MarketingGroupCardResponse> future = CompletableFuture.supplyAsync(() -> {
                //异步线程计算营销业务
                return getMarketingGroupCardResponse(request, stopWatch, customer, purchaseResponse, groupedInfoVos);
            }, orderExecutor);
        ...
        // 获取每个任务的返回结果
        for(CompletableFuture<com.wanmi.sbc.returnorder.api.response.purchase.MarketingGroupCardResponse> future : futureList){
            try {
                com.wanmi.sbc.returnorder.api.response.purchase.MarketingGroupCardResponse groupCardResponse = future.get();
                marketingGroupCardResponseList.add(groupCardResponse);
            } catch (Exception e) {
                throw new SbcRuntimeException("多线程计算营销处理业务失败");
            }
        }
        stopWatch.stop();
        groupCartMallResponse.setMarketingGroupCardResponseList(marketingGroupCardResponseList);

        System.out.println(stopWatch.prettyPrint());
        return BaseResponse.success(groupCartMallResponse);
    }

    private void calcNoJoinMarketTotalPrice(List<DevanningGoodsInfoVO> noJoinMarketingGoods, MarketingGroupCardResponse marketingGroupCardResponse) {
        //计算未参加营销总价
     ...
     * 设置库存及goodsstatus
     */
    private void setStockAndGoodsStatus(List<DevanningGoodsInfoVO> devanningGoodsInfoVOS, Long wareId){
        ...
        if(CollectionUtils.isNotEmpty(devanningGoodsInfoVOS)){
            List<String> skuIds = devanningGoodsInfoVOS.stream().map(DevanningGoodsInfoVO::getGoodsInfoId).collect(Collectors.toList());
        ...
        if(CollectionUtils.isNotEmpty(devanningGoodsInfoVOS)){
            ...
            devanningGoodsInfoVOS.forEach(var ->{
                setValue(var, finalStockMap,townStockMap);
            });
        }
    }

    private void setValue(DevanningGoodsInfoVO goodsInfo,Map<String, BigDecimal> stockMap,Map<String, BigDecimal> townStockMap){
        //设置库存信息 扣除乡镇件
    ...
    //购物车赋值是否参与囤货及囤货库存
    private void setPile(List<DevanningGoodsInfoVO> devanningGoodsInfoVOS ){
        if (CollectionUtils.isEmpty(devanningGoodsInfoVOS)){
            return ;
        }
        List<String> goodsInfoIds = devanningGoodsInfoVOS.stream().map(DevanningGoodsInfoVO::getGoodsInfoId).collect(Collectors.toList());
        PileActivityPileActivityGoodsRequest papagRequest = PileActivityPileActivityGoodsRequest.builder().goodsInfoIds(goodsInfoIds).build();

        //多商家囤货活动，设置是否参与囤货及囤货库存
        List<PileActivityGoodsVO> pagVOList = pileActivityProvider.getStartPileActivityPileActivityGoods(papagRequest).getContext();
        if (CollectionUtils.isEmpty(pagVOList)){
            return;
        }

        Map<String, PileActivityGoodsVO> pagVOMap = pagVOList.stream().collect(Collectors.toMap(PileActivityGoodsVO::getGoodsInfoId, y -> y, (a, b) -> a));
        devanningGoodsInfoVOS.forEach(v->{
            //设置虚拟库存
            if(pagVOMap.containsKey(v.getGoodsInfoId())){
                PileActivityGoodsVO pagVO = pagVOMap.get(v.getGoodsInfoId());
                v.setPileFlag(BoolFlag.YES);
                v.setVirtualStock(pagVO.getVirtualStock());
            }else{
                v.setPileFlag(BoolFlag.NO);
                v.setVirtualStock(0l);
            }
        });
    }
        ...
        for (DevanningGoodsInfoVO devanningGoodsInfoVO : devanningGoodsInfoVOS) {
            ...
            if(CollectionUtils.isEmpty(marketingPurchaseLimitVOS1)){
                ...
                if(Objects.nonNull(purchaseNum)){
                    ...
                    devanningGoodsInfoVO.setPurchaseNumOfMarketing(BigDecimal.valueOf(max));
                    continue;
                }
...
//        }
    ...
    public void setGoodsStaues(List<DevanningGoodsInfoVO> devanningGoodsInfoVOS,PurchaseListRequest request){
        List<Long> validStoreIds =getValidStoreIds(devanningGoodsInfoVOS);
        devanningGoodsInfoVOS.forEach(goodsInfoVO -> {
            if (validStoreIds.contains(goodsInfoVO.getStoreId())) {
                if (Objects.equals(DeleteFlag.NO, goodsInfoVO.getDelFlag())
                        && Objects.equals(CheckStatus.CHECKED, goodsInfoVO.getAuditStatus()) && Objects.equals(AddedFlag.YES.toValue(), goodsInfoVO.getAddedFlag())) {
                    goodsInfoVO.setGoodsStatus(GoodsStatus.OK);
                    // 判断是否有T，如果是1，就设置为2
                    ...
                    log.info("5 else flag");
                    goodsInfoVO.setGoodsStatus(GoodsStatus.INVALID);
                }
            }else {
                log.info("商品skuId[{}]skuName[{}]商家店铺状态不对", goodsInfoVO.getGoodsInfoId(), goodsInfoVO.getGoodsInfoName());
                goodsInfoVO.setGoodsStatus(GoodsStatus.INVALID);
            }
        });
        /**
         ...
         */
//        log.info("校验区域限购数量");
//        List<DevanningGoodsInfoMarketingVO> requestparam = KsBeanUtil.convert(devanningGoodsInfoVOS, DevanningGoodsInfoMarketingVO.class);
//        StockAndPureChainNodeRsponse context = purchaseProvider.checkStockPurchase(StockAndPureChainNodeRequeest.builder()
//                .wareId(request.getWareId())
//                .provinceId(request.getProvinceId())
//                .cityId(request.getCityId())
//                .checkPure(requestparam).build()).getContext();
//        if(Objects.isNull(context) || CollectionUtils.isEmpty(context.getCheckPure())){
//            return;
//        }
//        List<DevanningGoodsInfoPureVO> checkPure = context.getCheckPure();
//        log.info("checkPure:{}",JSON.toJSONString(checkPure));
//        Map<String, DevanningGoodsInfoPureVO> collect = checkPure.stream().collect(Collectors.toMap(DevanningGoodsInfoPureVO::getGoodsInfoId, y -> y, (a, b) -> a));
//
//        devanningGoodsInfoVOS.forEach(goodsInfoVO -> {
//            String goodsInfoId = goodsInfoVO.getGoodsInfoId();
//            GoodsStatus goodsStatus = goodsInfoVO.getGoodsStatus();
//            // 跳过不正常的商品状态
//            if(!GoodsStatus.OK.equals(goodsStatus)){
//                return;
//            }
//            DevanningGoodsInfoPureVO devanningGoodsInfoPureVO = collect.get(goodsInfoId);
//            if(Objects.isNull(devanningGoodsInfoPureVO)){
//                return;
//            }
//            // 限购
//            if (Objects.nonNull(devanningGoodsInfoPureVO.getType())){
//                log.info("校验区域限购数量:{}",devanningGoodsInfoPureVO.getGoodsInfoId());
//                goodsInfoVO.setGoodsStatus(GoodsStatus.QUOTA);
//            }
//        });
    }

    private List<Long> getValidStoreIds(List<DevanningGoodsInfoVO> devanningGoodsInfoVOS) {
        List<Long> goodsStoreIds = devanningGoodsInfoVOS.stream().map(DevanningGoodsInfoVO::getStoreId).distinct().collect(Collectors.toList());
        List<StoreVO> storeVOList = storeQueryProvider.listByIds(ListStoreByIdsRequest.builder().storeIds(goodsStoreIds).build()).getContext().getStoreVOList();
        List<Long> validStoreIds = storeVOList.stream().filter(s -> s.getDelFlag().toValue() == DeleteFlag.NO.toValue() && s.getStoreState().toValue() == StoreState.OPENING.toValue()).map(StoreVO::getStoreId).distinct().collect(Collectors.toList());
...
//        purchaseCountGoodsResponse.setPileTotal(pileTotal);
        PurchaseCountGoodsResponse purchaseCountGoodsResponse = new PurchaseCountGoodsResponse();
        purchaseCountGoodsResponse.setShopCartTotal(shopCartService.queryGoodsNumAndCache(request.getCustomerId(), request.getWareId(), request.getBulkWareId()));
        purchaseCountGoodsResponse.setPileTotal(shopCartNewPileTradeService.queryGoodsNumAndCache(request.getCustomerId(),request.getWareId()));
        if (Objects.nonNull(purchaseCountGoodsResponse.getShopCartTotal())){
            purchaseCountGoodsResponse.setTotal(purchaseCountGoodsResponse.getShopCartTotal().intValue());
        }
        if (Objects.nonNull(purchaseCountGoodsResponse.getPileTotal())){
            purchaseCountGoodsResponse.setTotal((Objects.nonNull(purchaseCountGoodsResponse.getTotal())?purchaseCountGoodsResponse.getTotal():0)+purchaseCountGoodsResponse.getPileTotal().intValue());
        }
