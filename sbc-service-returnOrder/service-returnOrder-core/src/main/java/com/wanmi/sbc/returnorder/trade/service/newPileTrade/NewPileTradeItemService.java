...
import com.wanmi.sbc.returnorder.trade.model.newPileTrade.NewPileTradeItemSnapshot;
import com.wanmi.sbc.returnorder.trade.model.root.TradeItemGroup;
import com.wanmi.sbc.returnorder.trade.repository.newPileTrade.NewPileTradeItemSnapshotRepository;
import com.wanmi.sbc.returnorder.trade.service.newPileTrade.update.NewPileTradeItemSnapshotService;
...
@Service
@Slf4j
public class NewPileTradeItemService {

    @Resource
    private NewPileTradeItemSnapshotRepository newPileTradeItemSnapshotRepository;

    @Autowired
    private NewPileTradeItemSnapshotService newPileTradeItemSnapshotService;
     ...
     */
    public NewPileTradeItemSnapshot findTradeItemSnapshot(String customerId) {
        Optional<NewPileTradeItemSnapshot> optional = newPileTradeItemSnapshotRepository.findByBuyerId(customerId);
    ...
    public List<TradeItemGroup> find(String customerId) {
        Optional<NewPileTradeItemSnapshot> optional = newPileTradeItemSnapshotRepository.findByBuyerId(customerId);
        return optional.map(NewPileTradeItemSnapshot::getItemGroups).orElseThrow(() -> new SbcRuntimeException("K-050201"));
    }
    ...
    public List<TradeItemGroup> findItemByCustomerId(String customerId) {
        Optional<NewPileTradeItemSnapshot> optional = newPileTradeItemSnapshotRepository.findByBuyerId(customerId);
        return optional.map(NewPileTradeItemSnapshot::getItemGroups).orElse(Lists.newArrayList());
    }
    ...
    public UnsatisfiedMarketingVO queryStockNum(String customerId, Long wareId, Long storeId) {
        NewPileTradeItemSnapshot tradeItemSnapshot = newPileTradeItemSnapshotRepository.findByBuyerId(customerId).orElse(null);
    ...
    public void updateUnStock(String customerId,Long wareId,Long storeId) {
        ...
        if (Objects.nonNull(tradeItemSnapshot)) {
            ...
            //更新订单快照
            newPileTradeItemSnapshotService.updateTradeItemSnapshot(tradeItemSnapshot);
        }
    }
    ...
    public void snapshot(TradeItemSnapshotRequest request, List<TradeItem> tradeItems, List<TradeMarketingDTO> tradeMarketingList,
        ...
        NewPileTradeItemSnapshot tradeItemSnapshot = newPileTradeItemSnapshotRepository.findByBuyerId(customerId).orElse(null);
        if(Objects.nonNull(tradeItemSnapshot)){
            newPileTradeItemSnapshotService.deleteTradeItemSnapshot(tradeItemSnapshot.getId());
        }
        //生成快照
        NewPileTradeItemSnapshot snapshot = NewPileTradeItemSnapshot.builder()
                .id(UUIDUtil.getUUID())
                .buyerId(customerId)
                ...
                .build();
        newPileTradeItemSnapshotService.addTradeItemSnapshot(snapshot);
    }
...
//            }
    ...
    public void remove(String customerId) {
        newPileTradeItemSnapshotRepository.deleteByBuyerId(customerId);
    }
    ...
    public NewPileTradeItemSnapshot findByCustomerId(String customerId) {
        return newPileTradeItemSnapshotRepository.findByBuyerId(customerId).orElse(null);
    }

    public void updateTradeItemSnapshotNoRollback(NewPileTradeItemSnapshot tradeItemSnapshot){
        newPileTradeItemSnapshotService.updateTradeItemSnapshotNoRollback( tradeItemSnapshot);
    }
