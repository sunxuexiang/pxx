...
import com.wanmi.sbc.returnorder.api.request.purchase.PurchaseFrontMiniRequest;
import com.wanmi.sbc.returnorder.api.request.purchase.PurchaseFrontRequest;
import com.wanmi.sbc.returnorder.api.request.purchase.PurchaseMergeRequest;
import com.wanmi.sbc.returnorder.api.request.purchase.PurchaseQueryRequest;
...
public class PilePurchaseService {
    ...
    public void save(PilePurchaseRequest request) {
        ...
        try {
            ...
            com.wanmi.sbc.returnorder.pilepurchase.PilePurchase purchase = pilePurchaseRepository.findOne(request.getWhereCriteria()).orElse(null);
    ...
    public void batchSave(PilePurchaseRequest request) {
        ...
        List<com.wanmi.sbc.returnorder.pilepurchase.PilePurchase> purchaseList = pilePurchaseRepository.findAll(PilePurchaseRequest.builder()
        ...
        goodsInfoList.forEach(goods -> {
            com.wanmi.sbc.returnorder.pilepurchase.PilePurchase purchases = purchaseList.stream().map(purchase -> {
...
//                }
    ...
    public void mergePurchase(PurchaseMergeRequest request) {
        ...
        List<com.wanmi.sbc.returnorder.pilepurchase.PilePurchase> purchaseList;
        ...
        goodsInfoList.forEach(goods -> {
            // 存在重复商品 替换采购单购买数量
            Optional<com.wanmi.sbc.returnorder.pilepurchase.PilePurchase> purchaseOptional =
                    purchaseList.stream().filter(p -> StringUtils.equals(goods.getGoodsInfoId(), p.getGoodsInfoId())).findFirst();
            if (purchaseOptional.isPresent()) {
                com.wanmi.sbc.returnorder.pilepurchase.PilePurchase purchase = purchaseOptional.get();
                Long goodsNum =
...
//                        .goodsNum(goodsNum).isCover(true).build());
        ...
        if (goodsInfos.size() > 0) {
            if (goodsInfos.size() == Constants.PURCHASE_MAX_SIZE && purchaseList.size() > 0) {
                pilePurchaseRepository.deleteByGoodsInfoids(purchaseList.stream().map(com.wanmi.sbc.returnorder.pilepurchase.PilePurchase::getGoodsInfoId).collect(Collectors.toList()), customer.getCustomerId(), request.getInviteeId());
                ...
                if (num >= purchaseList.size() && purchaseList.size() > 0) {
                    ...
                    pilePurchaseRepository.deleteByGoodsInfoids(purchaseList.subList(purchaseList.size() - num,
                            purchaseList.size()).stream()
                                    .map(com.wanmi.sbc.returnorder.pilepurchase.PilePurchase::getGoodsInfoId).collect(Collectors.toList()),
...
//        }
        purchase = new com.wanmi.sbc.returnorder.pilepurchase.PilePurchase();
     ...
     * @param request
     */
    private void updatePurchase(com.wanmi.sbc.returnorder.pilepurchase.PilePurchase purchase, PilePurchaseRequest request) {
...
//        response.setGoodsIntervalPrices(priceResponse.getGoodsIntervalPriceVOList());
    ...
    public MiniPurchaseResponse miniList(PilePurchaseRequest request, CustomerDTO customer) {
        ...
        Page<com.wanmi.sbc.returnorder.pilepurchase.PilePurchase> purchases = pilePurchaseRepository.findAll(request.getWhereCriteria(), request.getPageRequest());
        MiniPurchaseResponse miniPurchaseResponse = new MiniPurchaseResponse();
        miniPurchaseResponse.setGoodsList(Collections.EMPTY_LIST);
        miniPurchaseResponse.setGoodsIntervalPrices(Collections.EMPTY_LIST);
        miniPurchaseResponse.setPurchaseCount(0);
        ...
        goodsInfoRequest.setGoodsInfoIds(purchases.getContent().stream().map(com.wanmi.sbc.returnorder.pilepurchase.PilePurchase::getGoodsInfoId).collect(Collectors.toList()));
        //需要显示规格值
        goodsInfoRequest.setIsHavSpecText(Constants.yes);
...
//                            goodsInfo.getGoodsId(), goodsInfo.getGoodsInfoId());
    ...
    public PurchaseResponse pageList(PilePurchaseRequest request) throws SbcRuntimeException {
        ...
        try {
            ...
            List<com.wanmi.sbc.returnorder.pilepurchase.PilePurchase> follows = new ArrayList<>();
            GoodsInfoViewByIdsResponse response = null;
            // 查询分页数据，防止前端对象太多，不是初始化的时候只查商品的分页数据
            Page<com.wanmi.sbc.returnorder.pilepurchase.PilePurchase> purchasePage = null;
            ...
            if (request.getIsRefresh()) {
                ...
                GoodsInfoViewByIdsRequest goodsInfoRequest = new GoodsInfoViewByIdsRequest();
                goodsInfoRequest.setGoodsInfoIds(follows.stream().map(com.wanmi.sbc.returnorder.pilepurchase.PilePurchase::getGoodsInfoId).collect(Collectors.toList()));
                //需要显示规格值
                goodsInfoRequest.setIsHavSpecText(Constants.yes);
                ...
                for(int i = 0; i < follows.size(); i++) {
                    com.wanmi.sbc.returnorder.pilepurchase.PilePurchase purchase = follows.get(i);
                ...
                updateList(follows);
                follows.sort(Comparator.comparing(com.wanmi.sbc.returnorder.pilepurchase.PilePurchase::getValidSort));
                List<com.wanmi.sbc.returnorder.pilepurchase.PilePurchase> purchaseList = follows.stream().limit(request.getPageSize()).collect(Collectors.toList());
                ...
                List<com.wanmi.sbc.returnorder.pilepurchase.PilePurchase> purchasePageList = purchasePage.getContent();
                if (purchasePageList.size() == 0) {
                    return emptyResponse;
                }
                follows = purchasePageList;
                GoodsInfoViewByIdsRequest goodsInfoRequest = new GoodsInfoViewByIdsRequest();
                goodsInfoRequest.setGoodsInfoIds(purchasePage.getContent().stream().map(com.wanmi.sbc.returnorder.pilepurchase.PilePurchase::getGoodsInfoId).collect(Collectors.toList()));
                //需要显示规格值
                goodsInfoRequest.setIsHavSpecText(Constants.yes);
                goodsInfoRequest.setWareId(request.getWareId());
                goodsInfoRequest.setMatchWareHouseFlag(request.getMatchWareHouseFlag());
...
//        });
            ...
            //采购单中skuId
            List<String> goodsIds = follows.stream().map(com.wanmi.sbc.returnorder.pilepurchase.PilePurchase::getGoodsId).distinct().collect(Collectors.toList());
            List<GoodsVO> goodsList = response.getGoodses().stream()
            ...
            List<GoodsInfoVO> goodsInfoList = IteratorUtils.zip(response.getGoodsInfos(), follows,
                    (GoodsInfoVO sku, com.wanmi.sbc.returnorder.pilepurchase.PilePurchase f) -> sku.getGoodsInfoId().equals(f.getGoodsInfoId()),
                    (GoodsInfoVO sku, com.wanmi.sbc.returnorder.pilepurchase.PilePurchase f) -> {
            ...
            if(request.getIsRefresh()) {
                List<String> goodsInfoPageIds = purchasePage.getContent().stream()
                        .map(PilePurchase::getGoodsInfoId)
                        .collect(Collectors.toList());
                List<String> goodsPageIds = purchasePage.getContent().stream()
                        .map(PilePurchase::getGoodsId)
                        .distinct()
                        .collect(Collectors.toList());
                ...
                // 购物车商品
                for(PilePurchase purchase : follows) {
    ...
    public PurchaseResponse list(PilePurchaseRequest request) throws SbcRuntimeException {
        ...
        try {
            ...
            GoodsInfoViewByIdsRequest goodsInfoRequest = new GoodsInfoViewByIdsRequest();
            goodsInfoRequest.setGoodsInfoIds(follows.stream().map(PilePurchase::getGoodsInfoId).collect(Collectors.toList()));
            //需要显示规格值
            goodsInfoRequest.setIsHavSpecText(Constants.yes);
            goodsInfoRequest.setWareId(request.getWareId());
            goodsInfoRequest.setMatchWareHouseFlag(request.getMatchWareHouseFlag());
...
//        });
            ...
            //采购单中skuId
            List<String> goodsIds = follows.stream().map(PilePurchase::getGoodsId).distinct().collect(Collectors.toList());
            List<GoodsVO> goodsList = response.getGoodses().stream()
                    .filter(goods -> goodsIds.contains(goods.getGoodsId()))
                    .map(goods -> {
                        goods.setGoodsDetail("");
            ...
            List<String> goodsInfoIdList = new ArrayList<>();
            List<GoodsInfoVO> goodsInfoList = IteratorUtils.zip(response.getGoodsInfos(), follows,
                    (GoodsInfoVO sku, PilePurchase f) -> sku.getGoodsInfoId().equals(f.getGoodsInfoId()),
                    (GoodsInfoVO sku, PilePurchase f) -> {
                        sku.setCreateTime(f.getCreateTime());
                        sku.setIsCheck(f.getIsCheck());
                        goodsInfoIdList.add(sku.getGoodsInfoId());
                    });
    ...
    public void updateList(List<PilePurchase> purchases) {
    ...
    public void updateIsCheck(PilePurchaseRequest request, List<String> goodsInfoIds) throws SbcRuntimeException {
        List<PilePurchase> purchases = pilePurchaseRepository.findAll(request.getWhereCriteria());
        List<PilePurchase> savePurchases = new ArrayList<>();
        for (PilePurchase purchase : purchases) {
         ...
         * @return
         */
    public List<PilePurchase> queryPurchase(String customerId, List<String> goodsInfoIds, String inviteeId) {
        //分页查询SKU信息列表
...
//    }
    ...
    public void updateNum(PilePurchaseRequest request) {
        ...
        List<PilePurchase> purchaseList = pilePurchaseRepository.findAll(PilePurchaseRequest.builder()
                .customerId(request.getCustomerId())
                .goodsInfoId(request.getGoodsInfoId())
        ...
        if (CollectionUtils.isNotEmpty(purchaseList)) {
            PilePurchase purchase = purchaseList.get(0);
            //查询囤货商品数量
            ...
            //如果数据不存在，自动加入采购单
            PilePurchase purchase = new PilePurchase();
    ...
    public void updateNumByIds(PilePurchaseRequest request) {
        ...
        if (CollectionUtils.isNotEmpty(purchaseList)) {
            ...
            List<PilePurchase> savePilePurchaseList = Lists.newArrayList();
            List<PilePurchase> deletePilePurchaseList = Lists.newArrayList();

            for (PilePurchase param : purchaseList) {
    ...
    public void addFollow(PilePurchaseRequest queryRequest) {
        ...
        List<PilePurchase> purchaseList =
    ...
    public void clearLoseGoods(String userId, DistributeChannel distributeChannel) {
        List<PilePurchase> purchaseList = pilePurchaseRepository.queryPurchaseByCustomerIdAndInviteeId(userId,
                distributeChannel.getInviteeId());
        //筛选商品id
        List<String> goodsIds = purchaseList.stream().map(PilePurchase::getGoodsInfoId).collect(Collectors.toList());
    ...
    public List<GoodsInfoVO> fillBuyCount(List<GoodsInfoVO> goodsInfoList, String customerId, String inviteeId) {
        //分页查询SKU信息列表
        List<PilePurchase> follows = pilePurchaseRepository.findAll(PilePurchaseRequest.builder()
     ...
     */
    public void fillBuyCount(List<GoodsInfoVO> goodsInfoList, List<PilePurchase> followList) {
        ...
        goodsInfoList.stream().forEach(goodsInfo -> {
            goodsInfo.setBuyCount(0L);
            PilePurchase purchase = followList.stream().filter(p -> goodsInfo.getGoodsInfoId().equals(p.getGoodsInfoId())).findFirst().orElse(null);
            if (Objects.nonNull(purchase)) {
...
//            }
     ...
     * @param followList    SKU商品
     */
    public void fillBuyCountInit(List<GoodsInfoVO> goodsInfoList, List<PilePurchase> followList, List<String> selectSkuids) {
        if (CollectionUtils.isEmpty(goodsInfoList) || CollectionUtils.isEmpty(followList)) {
            return;
        }
        //填充SKU的购买数
        goodsInfoList.stream().forEach(goodsInfo -> {
            ...
            if (CollectionUtils.isNotEmpty(selectSkuids)){
                if (selectSkuids.contains(goodsInfo.getGoodsInfoId())){
                    Optional<PilePurchase> first = followList.stream().filter(purchase -> goodsInfo.getGoodsInfoId().equals(purchase.getGoodsInfoId())).findFirst();
