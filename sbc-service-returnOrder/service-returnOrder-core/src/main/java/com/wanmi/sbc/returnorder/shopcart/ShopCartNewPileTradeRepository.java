...
@Repository
public interface ShopCartNewPileTradeRepository extends JpaRepository<ShopCartNewPileTrade, Long>, JpaSpecificationExecutor<ShopCartNewPileTrade> {
     ...
     * @return
     */
    @Query("select p from ShopCartNewPileTrade p where p.goodsInfoId in (:goodsInfoIds) and p.customerId = :customerId and p.inviteeId=:inviteeId")
    List<ShopCartNewPileTrade> queryPurchaseByGoodsIdsAndCustomerId(@Param("goodsInfoIds") List<String> goodsInfoIds, @Param
    ...
    List<ShopCartNewPileTrade> queryPurchaseByWareIdAndCustomerId(@Param("wareId") Long wareId, @Param
