...
@Slf4j
@Service
public class EsBulkGoodsInfoElasticService {
...
//    private EsGoodsInfoElasticService esGoodsInfoElasticService;
    ...
    @Autowired
    private EsBulkGoodsInfoElasticRepository esBulkGoodsInfoElasticRepository;

    @Autowired
    private EsBulkGoodsElasticRepository esBulkGoodsElasticRepository;
     ...
     * @return
     */
    public EsGoodsResponse pageByGoods(EsBulkGoodsInfoQueryRequest queryRequest) {
        EsGoodsResponse goodsResponse = EsGoodsResponse.builder().build();
        if (StringUtils.isNotBlank(queryRequest.getKeywords())) {
            queryRequest.setKeywords(analyze(queryRequest.getKeywords()));
        }

        if (queryRequest.getCateId() != null) {
            //分类商品
            queryRequest = this.wrapperGoodsCateToEsGoodsInfoQueryRequest(queryRequest);
        }

        //店铺分类，加入所有子分类
        if (CollectionUtils.isNotEmpty(queryRequest.getStoreCateIds())) {
            queryRequest = this.wrapperStoreCateToEsGoodsInfoQueryRequest(queryRequest);
        }

        //设定排序
        if (queryRequest.getSortFlag() != null) {
            queryRequest = this.wrapperSortToEsGoodsInfoQueryRequest(queryRequest);
        }
...
//        );
        ...
        List<GoodsVO> goodses = bulkGoodsQueryProvider.bulkListByCondition(goodsQueryRequest).getContext().getGoodsVOList();

        Map<String, GoodsInfoVO> goodsInfoMap = bulkGoodsInfoQueryProvider.listGoodsInfoAndStcokByIds(
...
//                }
        ...
        //提取聚合数据
        EsGoodsBaseResponse baseResponse = this.extractBrands(response);
        goodsResponse.setBrands(baseResponse.getBrands());

        if (queryRequest.isCateAggFlag()) {
            //提取聚合数据
            baseResponse = this.extractGoodsCate(response);
            goodsResponse.setCateList(baseResponse.getCateList());
        }

        //提取聚合数据
        goodsResponse.setBrands(this.extractBrands(response).getBrands());

        //提取规格与规格值聚合数据
        baseResponse = this.extractGoodsSpecsAndSpecDetails(response);
     ...
     * @return
     */
    public EsBulkGoodsInfoQueryRequest wrapperGoodsCateToEsGoodsInfoQueryRequest(EsBulkGoodsInfoQueryRequest queryRequest) {
    ...
    public void initEsBulkGoodsInfo(EsGoodsInfoRequest request) {
        ...
        Map<String, List<GoodsAttributeKeyVO>> goodsInfoSpuMap = new HashMap<>();
        Map<String, List<GoodsAttributeKeyVO>> goodsPropDetailSkuMap = new HashMap<>();
        ...
        for (int i = 0; i < pageCount; i++) {
            ...
            if (CollectionUtils.isNotEmpty(goodsInfopage.getContent())) {
                ...
                //属性值Map
//                goodsPropDetailSkuMap.putAll(goodsAttributeKeyQueryProvider.arrListByGoodsIds(storeCateGoodsRelaListByGoodsIdsRequest).getContext().getAttributeKeyVOS().stream()
//                        .map(price -> KsBeanUtil.convert(price, GoodsAttributeKeyVO.class))
//                        .collect(Collectors.groupingBy(GoodsAttributeKeyVO::getGoodsInfoId)));
//
//                //属性值Map
//                goodsInfoSpuMap.putAll(goodsAttributeKeyQueryProvider.arrListByGoodsIds(storeCateGoodsRelaListByGoodsIdsRequest).getContext().getAttributeKeyVOS().stream()
//                        .map(price -> KsBeanUtil.convert(price, GoodsAttributeKeyVO.class))
//                        .collect(Collectors.groupingBy(GoodsAttributeKeyVO::getGoodsId)));
//                //商品店铺分类Map
...
//                List<EsCateBrand> esCateBrands = new ArrayList<>();
                goodsInfopage.getContent().stream().filter(goodsInfo -> goodses.containsKey(goodsInfo.getGoodsId())).forEach(goodsInfo -> {
                    ...
                    //定义文档结构
                    EsBulkGoodsInfo esGoodsInfo = new EsBulkGoodsInfo();
...
//                    }
                    ...
                    if (CollectionUtils.isNotEmpty(detailRels)) {
                        ...
                        esGoodsInfo.setSpecDetails(Lists.newArrayList());
                    }
                    ...
                    if (CollectionUtils.isNotEmpty(goodsPropDetailMap.get(goodsInfo.getGoodsId()))) {
                        esGoodsInfo.setPropDetails(goodsPropDetailMap.get(goodsInfo.getGoodsId()).stream().map(rel -> {
                            GoodsPropDetailRelNest relES = new GoodsPropDetailRelNest();
                    ...
                    //分配规格值(新的属性关系)
                    if (!goodsInfoSpecDetailMap.isEmpty()) {
                        esGoodsInfo.setGoodsAttributeSpu(goodsInfoSpuMap.get(goods.getGoodsInfoIds()));
                    }
...
//        this.updateStockGoodsInfo(stockVOList);
    ...
    private Map<String, List<GoodsPropDetailRelVO>> getPropDetailRelList(List<String> goodsIds) {
        ...
        List<GoodsPropDetailRelVO> goodsPropDetailRelVOList =
                bulkGoodsQueryProvider.getRefByGoodIds(relByIdsRequest).getContext().getGoodsPropDetailRelVOList();
    ...
    public void delete(EsGoodsInfoQueryRequest queryRequest) {
        Iterable<EsBulkGoodsInfo> esGoodsInfoList = esBulkGoodsInfoElasticRepository.search(queryRequest.getWhereCriteria());
        if (esGoodsInfoList != null) {
            esGoodsInfoList.forEach(esGoodsInfo ->
                    client.prepareDelete()
                            .setIndex(EsConstants.DOC_BULK_GOODS_INFO_TYPE)
//                            .setIndex(EsConstants.INDEX_NAME)
                            .setType(EsConstants.DOC_BULK_GOODS_INFO_TYPE)
//                            .setParent(esGoodsInfo.getCateBrandId())
                            .setId(esGoodsInfo.getId()).execute().actionGet());
        }
        queryRequest.setQueryGoods(true);
        Iterable<EsBulkGoods> esGoodsList = esBulkGoodsElasticRepository.search(queryRequest.getWhereCriteria());
        elasticsearchTemplate.refresh(EsBulkGoodsInfo.class);
        if (esGoodsList != null) {
            esGoodsList.forEach(esGoods -> {
                client.prepareDelete()
                        .setIndex(EsConstants.DOC_BULK_GOODS_TYPE)
//                        .setIndex(EsConstants.INDEX_NAME)
                        .setType(EsConstants.DOC_BULK_GOODS_TYPE)
//                        .setParent(esGoods.getCateBrandId())
    ...
    public void modifyGoodsSeqNum(GoodsVO goodsVO) {
        if (Objects.nonNull(goodsVO)) {
            ...
            updateByQuery = updateByQuery.source(EsConstants.DOC_BULK_GOODS_TYPE)
                    //查询要修改的结果集
            ...
            updateByQuery2 = updateByQuery2.source(EsConstants.DOC_BULK_GOODS_INFO_TYPE)
                    //查询要修改的结果集
    ...
    public void updateAddedStatus(Integer addedFlag, List<String> goodsIds, List<String> goodsInfoIds,Integer goodsInfoType) {
        ...
        Map<String, GoodsInfoVO> goodsInfoMap =
                bulkGoodsInfoQueryProvider.listByCondition(infoQueryRequest).getContext().getGoodsInfos().stream().collect(Collectors.toMap(GoodsInfoVO::getGoodsInfoId, c -> c));
        if (esGoodsInfoList != null) {
            esGoodsInfoList.forEach(esGoodsInfo -> {
...
//                    iq.setParentId(item.getCateBrandId());
    ...
    public void delBrandIds(List<Long> brandIds, Long storeId) {
        deleteBrandNameCommon(brandIds, EsConstants.DOC_BULK_GOODS_TYPE, storeId);
        deleteBrandNameCommon(brandIds, EsConstants.DOC_BULK_GOODS_INFO_TYPE, storeId);
    }
    ...
    private Long deleteBrandNameCommon(List<Long> ids, String indexName, Long storeId) {
        ...
        String queryStoreName = StringUtils.equals(EsConstants.DOC_BULK_GOODS_TYPE, indexName) ? "goodsInfos.storeId" :
                "goodsInfo.storeId";
        Long resCount = 0L;
    ...
    private Long updateCateNameCommon(GoodsCateVO goodsCateVO) {
        ...
        if (Objects.nonNull(goodsCateVO)) {
            ...
            updateByQuery = updateByQuery.source(EsConstants.DOC_BULK_GOODS_INFO_TYPE, EsConstants.DOC_BULK_GOODS_TYPE)
                    //查询要修改的结果集
     ...
     * @return
     */
    public EsBulkGoodsInfoQueryRequest wrapperStoreCateToEsGoodsInfoQueryRequest(EsBulkGoodsInfoQueryRequest queryRequest) {
     ...
     * @return
     */
    public EsBulkGoodsInfoQueryRequest wrapperSortToEsGoodsInfoQueryRequest(EsBulkGoodsInfoQueryRequest queryRequest) {
//        if (StringUtils.isNotBlank(queryRequest.getKeywords())) {
//            queryRequest.putScoreSort();
//        }
        if (queryRequest.getSortFlag()!=11 && queryRequest.getSortFlag() != 2 && queryRequest.getSortFlag() != 3 && queryRequest.getSortFlag() != 4){
            if(queryRequest.isQueryGoods()){
                //根据设置的排序值排序
                if(queryRequest.getIsKeywords()){
...
//                    }
            ...
            case 12:
                //按照商品编辑排序升序
                queryRequest.getSorts().removeAll(queryRequest.getSorts());
                queryRequest.putSort("recommendSort",SortOrder.ASC);
                break;
            default:
                break;
        }
